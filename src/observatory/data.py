...
def get_topology_snapshot() -> dict[str, object]:
    """Return a point-in-time topology snapshot with a current timestamp."""
    return {
        "nodes": deepcopy(_SEED_NODES),
        "edges": deepcopy(_SEED_EDGES),
        "timestamp": datetime.now(UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
    }


def get_events() -> list[dict[str, object]]:
    """Return a copy of the current recent-event seed log."""
    return deepcopy(_SEED_EVENTS)
