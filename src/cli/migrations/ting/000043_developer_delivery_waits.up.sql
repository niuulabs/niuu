CREATE TABLE IF NOT EXISTS developer_delivery_waits (
    id UUID PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_developer_delivery_waits_active_generation
    ON developer_delivery_waits (execution_id, execution_generation)
    WHERE state <> 'notified';

CREATE INDEX IF NOT EXISTS idx_developer_delivery_waits_due
    ON developer_delivery_waits (state, next_poll_at, lease_expires_at)
