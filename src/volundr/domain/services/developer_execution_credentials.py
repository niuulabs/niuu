...
DEVELOPER_COORDINATE_SCOPE = "ting:developer:coordinate"
...
class DeveloperExecutionCredentialError(ValueError):
    """The configured credential projection cannot safely serve a session."""


@dataclass(frozen=True)
class DeveloperExecutionCredentialBinding:
    ...
    @classmethod
    def from_session(cls, session: Session) -> DeveloperExecutionCredentialBinding | None:
        provenance = session.workload_config.get("provenance")
        if not isinstance(provenance, dict):
            return None
        raw = provenance.get("developer_execution")
        if raw is None:
            return None
        if not isinstance(raw, dict):
            raise DeveloperExecutionCredentialError(
                "developer_execution provenance must be an object"
            )
        ...
        def required(name: str) -> str:
            ...
            if not value:
                raise DeveloperExecutionCredentialError(
                    f"developer_execution provenance requires {name}"
                )
            return value
        ...
        try:
            ...
            raise DeveloperExecutionCredentialError(
                "developer_execution execution_id must be a UUID"
            ) from exc
        ...
        if bool(attempt) != bool(intent) or bool(attempt) != bool(task):
            raise DeveloperExecutionCredentialError(
                "child developer_execution provenance requires attempt, intent, and task lineage"
            )
        try:
            ...
            raise DeveloperExecutionCredentialError(
                "developer_execution child attempt and intent ids must be UUIDs"
            ) from exc
        ...
        if not parent_session_key.startswith(expected_prefix):
            raise DeveloperExecutionCredentialError(
                f"developer_execution parent_session_key must start with {expected_prefix}"
            )
        return cls(
...
class DeveloperExecutionCredentialService:
    ...
    def __init__(
        ...
        *,
        ...
        projection: DeveloperCredentialProjectionPort,
        runtime_backend: str,
        ...
        self._locations: dict[UUID, DeveloperCredentialProjection] = {}

    async def project(self, session: Session) -> DeveloperCredentialProjection | None:
    ...
    def projection(self, session_id: UUID) -> DeveloperCredentialProjection:
    ...
    @staticmethod
    def _claims(
        binding: DeveloperExecutionCredentialBinding,
        forge_session_id: UUID,
    ) -> dict[str, Any]:
        claims: dict[str, Any] = {
            "scopes": [DEVELOPER_COORDINATE_SCOPE],
            "developer_execution_id": str(binding.execution_id),
