"""Explicit mini-mode compatibility for file-backed Codex subscription grants.

Production uses OpenBaoCodexCredentialBroker, which never calls this adapter.
"""
...
class MiniModeCodexCredentialBroker(CodexCredentialBrokerPort):
    """Explicit compatibility for mini mode; never usable by production brokers."""

    def __init__(
        self,
        *,
        credential_store: CredentialStorePort,
        refresh_lock: CredentialRefreshLockPort | None,
        mini_mode: bool,
        ...
        **_extra: object,
        ...
        if not mini_mode or refresh_lock is None:
            raise ValueError("Mini-mode Codex renewal requires mini mode and a refresh lock")
    ...
    async def get_tokens(
        ...
        *,
        ...
        async with self._refresh_lock.hold("user", owner_id, credential_name):
            ...
            if stored is None or stored.metadata.get("tenant_id") not in {None, "", tenant_id}:
                raise CodexCredentialBrokerError("Codex credential is unavailable for this caller")
            if stored.metadata.get("renewal_owner"):
                raise CodexCredentialBrokerError("Mini mode cannot refresh an engine-managed grant")
            ...
            try:
                auth = parse_codex_auth_document(raw_auth, require_refresh=True)
            ...
            if forced_refresh_is_still_current or remaining <= self._refresh_skew_seconds:
                ...
                await self._credential_store.store(
                    ...
                    updated_values,
                    codex_auth_metadata(
                        {**stored.metadata, "tenant_id": tenant_id},
