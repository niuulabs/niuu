...
DEFAULT_CONTAINER_DIRECTORY = "/var/run/secrets/niuu-developer-execution"


class FileDeveloperCredentialProjection(DeveloperCredentialProjectionPort):
    ...
    def __init__(
        ...
        **_extra: object,
        ...
        if runtime_backend not in {"local", "process", "docker"}:
            raise ValueError(
                "FileDeveloperCredentialProjection supports only local, process, and docker"
            )
    ...
    async def project(
        ...
        *,
        ...
        if runtime_backend != "docker":
            return DeveloperCredentialProjection(token_file=str(directory / "token"))
