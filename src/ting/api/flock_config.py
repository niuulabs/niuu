...
separate from operator settings under ``/api/v1/ting/settings/flock``.
"""
...
def create_flock_config_router() -> APIRouter:
    router = APIRouter(prefix="/api/v1/ting/flock", tags=["Flock"])
    ...
    async def patch_flock_config(
        ...
        if body.flock_default_personas is not None:
            from ting.config import PersonaOverride
