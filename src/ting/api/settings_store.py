"""Shared in-process storage for Ting operator settings routes.

These settings are intentionally mounted behind ``/api/v1/ting/settings/*`` and
kept distinct from runtime workflow config like ``/api/v1/ting/flock/config``.
Until Ting grows a dedicated settings repository, we persist changes in
...
def _seed_settings_store(settings: Settings) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "notifications": {
            ...
            "on_run_pending_approval": bool(settings.notification.enabled),
            "on_run_merged": bool(settings.notification.enabled),
            "on_run_failed": bool(settings.notification.enabled),
...
def get_ting_settings_store(request: Request) -> dict[str, dict[str, Any]]:
    """Return the process-local Ting settings store, seeding it once if needed."""
    existing = getattr(request.app.state, "ting_http_settings", None)
    if existing is None:
        existing = _seed_settings_store(request.app.state.settings)
        request.app.state.ting_http_settings = existing
    return existing
