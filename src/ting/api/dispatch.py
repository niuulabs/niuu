"""REST API for the dispatcher — queue and approve runs for execution.
...
class DispatchBatchRequest(BaseModel):
    """Compatibility request for dispatching runs by run tracker ID."""
...
async def _resolve_dispatch_item_for_run(
    run_id: str,
    *,
    trackers: list,
    ...
    for tracker in trackers:
        ...
        issue = next(
            (
                item
                for item in issues
                if item.id == run.tracker_id or item.identifier == run.tracker_id
            ),
            None,
        )
        if issue is None:
            return None, "run issue not found in tracker"
...
def create_dispatch_router() -> APIRouter:
    router = APIRouter(prefix="/api/v1/ting/dispatch", tags=["Dispatcher"])
    ...
    async def dispatch_batch(
        ...
        """Compatibility alias for dispatching multiple runs by run tracker ID."""
        await dispatcher_repo.get_or_create(principal.user_id)

        dispatch_items: list[ServiceDispatchItem] = []
        requested_run_by_issue_id: dict[str, str] = {}
        failures: list[FailedRunResponse] = []
        for run_id in body.run_ids:
            item, reason = await _resolve_dispatch_item_for_run(run_id, trackers=trackers)
            if item is None:
                failures.append(
                    FailedRunResponse(
                        run_id=run_id,
                        reason=reason or "run not found",
                    )
                )
                continue
            dispatch_items.append(item)
            requested_run_by_issue_id[item.issue_id] = run_id
        ...
        dispatched = [
            requested_run_by_issue_id[result.issue_id]
            for result in results
            if result.issue_id in requested_run_by_issue_id
        ]
    ...
    async def dispatch_run(
        ...
        """Compatibility alias for dispatching a single run by run tracker ID."""
        await dispatcher_repo.get_or_create(principal.user_id)
        ...
        if item is None:
            raise HTTPException(
                ...
                detail=reason or "run not found",
            )
