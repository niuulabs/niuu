...
class CreatePipelineRequest(BaseModel):
    """Request body for POST /api/v1/ting/pipelines."""
    ...
    auto_start: bool = Field(
        default=True,
        description="When True, dispatch Phase 1 runs immediately after creation.",
    )


class CreatePipelineResponse(BaseModel):
    """Response body for POST /api/v1/ting/pipelines."""
...
def create_pipelines_router() -> APIRouter:
    router = APIRouter(prefix="/api/v1/ting/pipelines", tags=["Pipelines"])
    ...
    async def create_pipeline(
        ...
        Parses the YAML, creates a Saga with Phases and Runs in the database,
        and (when ``auto_start`` is True) immediately dispatches Phase 1 runs
        to Volundr.
