"""Compile visual workflow DAGs into Ting pipeline YAML definitions."""
...
def compile_workflow_definition(workflow: WorkflowDefinition) -> WorkflowCompileResult:
    """Compile a stored workflow definition into Ting pipeline YAML."""
...
def compile_workflow_graph(name: str, graph: dict[str, Any]) -> WorkflowCompileResult:
    """Compile a workflow DAG into a linear Ting pipeline definition.

    The current Ting runtime supports a linear sequence of stages, with each
    ...
    if cond_nodes:
        ...
        errors.append(
            f"Conditional nodes are not yet executable in Ting runtime: {labels}.",
        )
...
def _linear_runtime_nodes(
    *,
    ...
    while True:
        ...
        if len(next_edges) > 1:
            errors.append(
                f"Node '{_node_label(current)}' fans out to multiple paths, which Ting "
                "cannot execute yet.",
            )
            break
        ...
        if next_node.get("kind") not in {"stage", "gate"}:
            errors.append(
                f"Node '{_node_label(next_node)}' is not executable in Ting runtime.",
            )
            break

        if len(incoming.get(next_id, [])) > 1:
            errors.append(
                f"Node '{_node_label(next_node)}' has multiple inbound edges, which Ting "
                "cannot execute yet.",
            )
            break
    ...
    expected_runtime_ids = {
        str(node["id"]) for node in node_map.values() if node.get("kind") in {"stage", "gate"}
    }
