...
def build_flock_workload_config(
    ...
    ``tpl_run.persona_overrides`` to the matching persona, and packages
    the result for use in a :class:`~ting.ports.volundr.SpawnRequest`.
    ...
    :param tpl_run: The run being dispatched (provides persona name and overrides).
    :param flow_provider: Provider used to resolve the flow definition.
    :param initial_prompt: The run prompt, stored as ``initiative_context``.
    ...
    if flow is None:
        logger.warning(
            "flock_merge: flow %r not found — dispatching run %r without flock config",
            flow_name,
            tpl_run.name,
        )
        return None
    ...
    # Apply per-run overrides to the matching persona
    if tpl_run.persona_overrides and tpl_run.persona:
        personas = [
            merge_persona_override(p, tpl_run.persona_overrides)
            if p.get("name") == tpl_run.persona
            else p
            for p in personas
        ]
