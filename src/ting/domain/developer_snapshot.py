...
def pinned_child_workflow(
    ...
    if child.execution_id != execution.id:
        raise DeveloperExecutionError("Child does not belong to this execution")
    ...
    if not isinstance(pin, dict) or not isinstance(aggregate, dict):
        raise DeveloperExecutionError("Execution has no frozen child workflow dependency")
    if (
        pin.get("id") != str(child.template_id)
        or pin.get("revision") != child.template_revision
        or pin.get("digest") != child.template_digest
    ):
        raise DeveloperExecutionError("Child pin differs from the frozen execution dependency")
    ...
    if (
        ...
        raise DeveloperExecutionError("Frozen child workflow does not match its content digest")
