...
class WorkflowWaitService:
    ...
    def __init__(
        ...
        *,
        ...
        execution_repository: DeliveryExecutionRepository,
        observer: WaitConditionObserver,
    ...
    async def request_wait(
        ...
        if execution.state in TERMINAL_EXECUTION_STATES or execution.cancel_requested:
            raise WorkflowExecutionError("Developer execution cannot register a delivery wait")
        if request.policy_id != self._policy_id:
            raise WorkflowExecutionError(
                "Delivery wait policy must match the configured integration policy"
            )
        if request.mode is WorkflowWaitMode.MERGE and request.provider_operation_id is None:
            raise WorkflowExecutionError(
                "Merge delivery waits require the provider operation ID returned by Forge"
            )
        if request.repository != execution.repository:
            raise WorkflowExecutionError("Delivery wait repository differs from the execution")
        ...
        if request.expected_base_sha != execution.base_sha:
            raise WorkflowExecutionError("Delivery wait base must match the execution base")
        ...
        wait_id = uuid5(
            NAMESPACE_URL,
            f"niuulabs:workflow-execution:{execution.id}:delivery-wait:{request_digest}",
        )
    ...
    async def list_waits(self, execution: DeliveryExecution) -> list[WorkflowWait]:
    ...
    async def _handle_wait_failure(
        ...
        *,
        ...
        status = (
            ...
            if wait.request.mode is WorkflowWaitMode.CHECKS
            else WaitObservationStatus.MERGE_FAILED
        )
    ...
    async def _reconcile_wait(self, wait: WorkflowWait, *, now: datetime) -> None:
        ...
        if execution is None:
            raise WorkflowExecutionError("Delivery wait execution no longer exists")
        ...
        if wait.state in {WorkflowWaitState.READY, WorkflowWaitState.FAILED}:
            if wait.observation is None or not wait.observation.terminal:
                raise WorkflowExecutionError("Terminal delivery wait is missing its observation")
            if (
