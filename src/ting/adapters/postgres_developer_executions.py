...
class PostgresDeveloperExecutionRepository(DeveloperExecutionRepository):
    ...
    async def create(self, execution: DeveloperExecution) -> DeveloperExecution:
        try:
            await self._pool.execute(
                """
                INSERT INTO developer_executions (
    ...
    async def get_internal(self, execution_id: UUID) -> DeveloperExecution | None:
        row = await self._pool.fetchrow(
            "SELECT * FROM developer_executions WHERE id = $1",
            execution_id,
        )
        return _execution_from_row(row) if row is not None else None
        ...
        *,
        ...
        async with self._pool.acquire() as connection, connection.transaction():
            row = await connection.fetchrow(
                "SELECT * FROM developer_executions WHERE id = $1 FOR UPDATE",
                execution.id,
            )
            if row is None:
            ...
            await connection.execute(
                """
                INSERT INTO developer_execution_generations (
            ...
            await connection.executemany(
                """
                INSERT INTO developer_execution_children (
    ...
    async def list_children(self, execution_id: UUID) -> list[ChildExecution]:
        rows = await self._pool.fetch(
            """
            SELECT * FROM developer_execution_children
    ...
    async def get_child(self, child_id: UUID) -> ChildExecution | None:
        row = await self._pool.fetchrow(
            "SELECT * FROM developer_execution_children WHERE id = $1",
            child_id,
        )
        ...
        *,
        ...
        async with self._pool.acquire() as connection, connection.transaction():
            executions = await connection.fetch(
                """
                SELECT execution.*
                FROM developer_executions execution
                WHERE execution.deadline > NOW()
                  ...
                  AND (execution.policy ->> 'maxActiveChildren')::INTEGER > (
                      SELECT COUNT(*)
                      FROM developer_execution_children active
                  ...
                  AND EXISTS (
                      SELECT 1 FROM developer_execution_children child
            ...
            for execution in executions:
                ...
                active = int(
                    await connection.fetchval(
                        """
                        SELECT COUNT(*)
                        FROM developer_execution_children
                        WHERE execution_id = $1
                          AND (
                ...
                rows = await connection.fetch(
                    ...
                    WITH claimable AS (
                        ...
                        WHERE child.execution_id = $1
                          ...
                          AND NOT EXISTS (
                              ...
                              WHERE NOT EXISTS (
                                  SELECT 1
                                  FROM developer_execution_children prerequisite
                                  WHERE prerequisite.execution_id = child.execution_id
                                    ...
                                    AND prerequisite.attempt = (
                                        ...
                                        FROM developer_execution_children latest
                    ...
                    UPDATE developer_execution_children child
    ...
    async def record_observation(
        ...
        async with self._pool.acquire() as connection, connection.transaction():
            parent = await connection.fetchrow(
                """
                SELECT id, cancel_requested FROM developer_executions
                WHERE id = $1
                FOR UPDATE
                """,
                child.execution_id,
            ...
            inserted = await connection.execute(
                """
                INSERT INTO developer_child_events (child_id, event_id, observed_at, state, payload)
            ...
            current_row = await connection.fetchrow(
                "SELECT * FROM developer_execution_children WHERE id = $1 FOR UPDATE",
                child.id,
            )
    ...
    async def seal_generation(self, execution_id: UUID, generation: int) -> None:
        result = await self._pool.execute(
            """
            UPDATE developer_execution_generations
    ...
    async def join_status(self, execution_id: UUID, generation: int):
        generation_row = await self._pool.fetchrow(
            """
            SELECT sealed FROM developer_execution_generations
            WHERE execution_id = $1 AND generation = $2
            """,
            execution_id,
            generation,
        ...
        *,
        ...
        async with self._pool.acquire() as connection, connection.transaction():
            ...
            latest_attempt = await connection.fetchval(
                """
                SELECT MAX(attempt) FROM developer_execution_children
        ...
        *,
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                "SELECT * FROM developer_executions WHERE id = $1 AND owner_id = $2 "
            ...
            children = await conn.fetch(
                "SELECT * FROM developer_execution_children WHERE execution_id = $1 "
            ...
            updated = await conn.fetchrow(
                "UPDATE developer_executions SET state = 'completed', "
    ...
    async def reserve_message(self, message: ChildMessage) -> ChildMessage:
        await self._pool.execute(
            """
            INSERT INTO developer_child_messages (
        ...
        row = await self._pool.fetchrow(
            "SELECT * FROM developer_child_messages WHERE message_id = $1",
            message.message_id,
        )
        if row is None:
    ...
    async def mark_message_delivered(self, message_id: UUID) -> None:
        await self._pool.execute(
            """
            UPDATE developer_child_messages
...
def _execution_from_row(row) -> DeveloperExecution:
