...
class ForgeWaitConditionObserver(WaitConditionObserver):
    def __init__(
        self,
        *,
    ...
    async def observe(
        ...
        if request.mode is WorkflowWaitMode.MERGE:
        ...
        if failures:
            status = WaitObservationStatus.CHECKS_FAILED
            ...
            status = WaitObservationStatus.CHECKS_PENDING
            reason = "Required Forge checks are not terminal"
        else:
            status = WaitObservationStatus.CHECKS_PASSED
    ...
    async def _observe_merge(self, execution, request, adapter, principal, auth_token):
        ...
        if receipt.state is PublicationState.QUEUED:
            return self._observation(
                request,
                WaitObservationStatus.MERGE_PENDING,
    ...
    @staticmethod
    def _observation(request, status, **kwargs) -> WaitObservation:
