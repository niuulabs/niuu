...
POST /api/v1/ting/telegram/webhook processes incoming Telegram messages,
authenticates the sender via notification_subscriptions (chat_id → owner_id),
and executes the requested command against Ting's domain services.
"""
...
HELP_TEXT = (
    ...
    "`/list [saga-slug] [limit]` — list runs ready for dispatch\n"
    "`/approve <id>` — approve a run in review\n"
    "`/reject <id> [reason]` — reject a run\n"
    "`/retry <id>` — re-dispatch a run\n"
    "`/pause` — pause the autonomous dispatcher\n"
    "`/resume` — resume the dispatcher\n"
    "`/dispatch <id> [model] [runtime]` — spawn a session for a ready run\n"
...
def parse_command(text: str) -> ParsedCommand | None:
    ...
    # Strip bot mention suffix (e.g. /status@TingBot)
    parts = text.split(None, 1)
    command_part = parts[0]
    # Strip bot mention suffix (e.g. /status@TingBot) — split on @ to avoid regex DoS
...
async def _handle_approve(
    ...
    *,
    tracker: TrackerPort,
    review_service: RunReviewService,
) -> str:
    if not cmd.args:
        return "*Usage:* `/approve <run-tracker-id>`"

    tracker_id = cmd.args[0]
    run = await _find_run_by_tracker_id(tracker, tracker_id, owner_id)
    if run is None:
        return f"Run not found: `{tracker_id}`"

    try:
        result = await review_service.approve(run.id)
    except InvalidRunStateError as exc:
        return f"Run `{tracker_id}` is in _{exc.current}_ state — can only approve from *REVIEW*"

    suffix = ""
    if result.phase_gate_unlocked:
        suffix = "\n_Phase gate unlocked_ — all runs in phase merged."
    return f"✅ Run `{tracker_id}` approved — status → *MERGED*{suffix}"
...
async def _handle_reject(
    ...
    *,
    ...
    if not cmd.args:
        return "*Usage:* `/reject <run-tracker-id> [reason]`"
    ...
    try:
        await review_service.reject(run.id, reason=reason)
    except InvalidRunStateError as exc:
        return f"Run `{tracker_id}` is in _{exc.current}_ state — can only reject from *REVIEW*"

    suffix = f" — reason: _{reason}_" if reason else ""
    return f"✗ Run `{tracker_id}` rejected — status → *FAILED*{suffix}"
...
async def _handle_retry(
    ...
    *,
    ...
    if not cmd.args:
        return "*Usage:* `/retry <run-tracker-id>`"
    ...
    try:
        result = await review_service.retry(run.id)
    except InvalidRunStateError as exc:
        return (
            f"Run `{tracker_id}` is in _{exc.current}_ state — "
            "can only retry from *REVIEW* or *FAILED*"
        )

    return f"↻ Run `{tracker_id}` queued for retry — status → *{result.run.status.value}*"
...
async def _handle_dispatch(
    ...
    *,
    ...
    """Spawn a Volundr session for a run, optionally with model + runtime overrides.
    ...
    # Find the ready issue. We rely on find_ready_issues so the run's phase
    ...
    if item is None:
        return (
            f"Run `{tracker_id}` is not ready for dispatch.\n"
            "Use `/list` to see what's ready, or check that prior-phase runs are merged."
        )
...
async def _handle_list(
    ...
    *,
    ...
    """List runs ready for dispatch, optionally scoped to a saga slug.
...
async def _find_run_by_tracker_id(
    ...
    """Locate a run by its tracker_id within the owner's sagas.

    Runs are identified by tracker_id (e.g. NIU-221) rather than internal UUID,
    so we search through the owner's sagas → phases → runs.
    Falls back to treating tracker_id as a UUID if the tracker supports get_run_by_id.
    """
    from uuid import UUID

    # Try direct UUID lookup as a fallback
    try:
        run_uuid = UUID(tracker_id)
        return await tracker.get_run_by_id(run_uuid)
    except ValueError:
        pass

    try:
        return await tracker.get_run(tracker_id)
    except Exception:
        return None
    ...
    async def telegram_webhook(request: Request) -> Response:
        ...
        if owner_id is None:
            await reply_client.send(
                chat_id,
                "This chat is not linked to a Ting account. "
                "Use the Ting web UI to set up Telegram notifications.",
            )
            return Response(status_code=status.HTTP_200_OK)
