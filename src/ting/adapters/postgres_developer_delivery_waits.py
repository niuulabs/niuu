...
from ting.ports.developer_delivery_wait import DeveloperDeliveryWaitRepository


class PostgresDeveloperDeliveryWaitRepository(DeveloperDeliveryWaitRepository):
    ...
    async def reserve(
        ...
        *,
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            ...
            if current is None:
                raise DeveloperExecutionError("Developer execution no longer exists")
            ...
            current_digest = developer_delivery_candidate_digest(
            ...
            active = await conn.fetchrow(
                """
                SELECT * FROM developer_delivery_waits
            ...
            row = await conn.fetchrow(
                """
                INSERT INTO developer_delivery_waits (
    ...
    async def claim_due(
        ...
        *,
        ...
        rows = await self._pool.fetch(
            ...
            UPDATE developer_delivery_waits AS waits
            SET lease_owner = $3,
    ...
    async def record_pending(
        ...
        *,
        ...
        result = await self._pool.execute(
            """
            UPDATE developer_delivery_waits
    ...
    async def record_terminal(
        ...
        *,
        ...
        if not observation.terminal:
            raise DeveloperExecutionError("Cannot terminally record a pending delivery observation")
        state = (
...
def _wait_from_row(row) -> DeveloperDeliveryWait:
    ...
    return DeveloperDeliveryWait(
        id=row["id"],
        execution_id=row["execution_id"],
        request=DeveloperDeliveryWaitRequest.model_validate(request_raw),
        request_digest=row["request_digest"],
        execution_generation=int(row["execution_generation"]),
        execution_revision=int(row["execution_revision"]),
        candidate_digest=row["candidate_digest"],
        state=DeveloperDeliveryWaitState(row["state"]),
        next_poll_at=row["next_poll_at"],
        observation=(
            DeveloperDeliveryObservation.model_validate(observation_raw)
            if observation_raw
            else None
        ),
