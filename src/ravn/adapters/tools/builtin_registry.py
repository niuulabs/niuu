...
BUILTIN_TOOLS: dict[str, BuiltinToolDef] = {
    ...
    "workflow_execution_expand": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionExpandTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "workflow_execution_reconcile": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionReconcileTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "workflow_execution_retry": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionRetryTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "workflow_execution_cancel": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionCancelTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "workflow_execution_message": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionMessageTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "workflow_execution_wait": BuiltinToolDef(
        adapter="ravn.adapters.tools.workflow_execution.WorkflowExecutionWaitTool",
        groups=frozenset({"workflow_execution"}),
        required_context=frozenset({"workflow_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["workflow_execution"]},
    ),
    "delivery_expand_workstreams": BuiltinToolDef(
        adapter="ravn.adapters.tools.delivery.DeliveryExpandWorkstreamsTool",
        groups=frozenset({"delivery"}),
        required_context=frozenset({"delivery_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["delivery_execution"]},
    ),
    "delivery_complete": BuiltinToolDef(
        adapter="ravn.adapters.tools.delivery.DeliveryCompleteTool",
        groups=frozenset({"delivery"}),
        required_context=frozenset({"delivery_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["delivery_execution"]},
    ),
    "delivery_record_integration": BuiltinToolDef(
        adapter=("ravn.adapters.tools.delivery.DeliveryRecordIntegrationTool"),
        groups=frozenset({"delivery"}),
        required_context=frozenset({"delivery_execution"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["delivery_execution"]},
    ),
    "delivery_workspace": BuiltinToolDef(
        adapter="ravn.adapters.tools.delivery.DeliveryWorkspaceTool",
        groups=frozenset({"delivery"}),
    ...
    "delivery_forge": BuiltinToolDef(
        adapter="ravn.adapters.tools.delivery.DeliveryForgeTool",
        groups=frozenset({"delivery"}),
        required_context=frozenset({"delivery_service"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["delivery_service"]},
    ),
    "delivery_evidence": BuiltinToolDef(
        adapter="ravn.adapters.tools.delivery.DeliveryEvidenceTool",
        groups=frozenset({"delivery"}),
        required_context=frozenset({"delivery_service"}),
        kwargs_fn=lambda _s, ctx: {"service": ctx["delivery_service"]},
    ),
