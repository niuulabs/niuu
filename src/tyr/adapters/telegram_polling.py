...
In production, Telegram calls Tyr's public webhook
(``POST /api/v1/tyr/telegram/webhook``). In dev, the platform usually isn't
...
_WEBHOOK_PATH = "/api/v1/tyr/telegram/webhook"
...
class TelegramPollingService:
    ...
    The ``self_url`` should be the loopback origin where Tyr's REST API is
    ...
    async def start(self) -> None:
        ...
        # Imported lazily so import failure doesn't block tyr startup when
        # polling is disabled.
        try:
        ...
        self._started = True
        logger.info("Telegram polling started — forwarding updates to %s%s",
                    self._self_url, _WEBHOOK_PATH)
