...
- dispatching one run to a tenant-scoped Volundr and one to a user-scoped Volundr
- verifying the spawned sessions through the central Guild API
"""
...
def main() -> int:
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        tenant_dispatch = _dispatch_one(
            ...
            slug=f"guild-tenant-dispatch-{suffix}",
            target_id=tenant_instance["id"],
        )
        user_dispatch = _dispatch_one(
            client,
            headers=tenant_b_headers,
            saga_name=f"Guild User Dispatch {suffix}",
            slug=f"guild-user-dispatch-{suffix}",
