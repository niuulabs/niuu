...
CREATE TABLE IF NOT EXISTS developer_executions (
    ...
    CONSTRAINT developer_executions_state_check CHECK (
        state IN ('pending', 'running', 'waiting', 'blocked', 'canceling',
                  'canceled', 'completed', 'failed')
    ),
    CONSTRAINT developer_executions_budget_check CHECK (
        reserved_budget + spent_budget <= total_budget
    )
);

CREATE INDEX IF NOT EXISTS idx_developer_executions_owner_updated
    ON developer_executions (owner_id, updated_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_developer_executions_active
    ON developer_executions (state, deadline)
    WHERE state IN ('pending', 'running', 'waiting', 'blocked', 'canceling');
CREATE UNIQUE INDEX IF NOT EXISTS idx_developer_executions_launch_key
    ON developer_executions (owner_id, tenant_id, launch_key)
    WHERE launch_key <> '';

CREATE TABLE IF NOT EXISTS developer_execution_generations (
    execution_id UUID NOT NULL REFERENCES developer_executions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS developer_execution_children (
    id UUID PRIMARY KEY,
    execution_id UUID NOT NULL REFERENCES developer_executions(id) ON DELETE CASCADE,
    generation INTEGER NOT NULL CHECK (generation > 0),
    ...
    FOREIGN KEY (execution_id, generation)
        REFERENCES developer_execution_generations(execution_id, generation) ON DELETE CASCADE,
    CONSTRAINT developer_children_state_check CHECK (
        state IN ('reserved', 'launching', 'submitted', 'running', 'waiting',
                  'blocked', 'canceling', 'canceled', 'completed', 'failed', 'superseded')
    )
);

CREATE INDEX IF NOT EXISTS idx_developer_children_execution
    ON developer_execution_children (execution_id, generation, child_key, attempt);
CREATE INDEX IF NOT EXISTS idx_developer_children_launch
    ON developer_execution_children (state, lease_expires_at, deadline)
    WHERE state IN ('reserved', 'launching');
CREATE INDEX IF NOT EXISTS idx_developer_children_task
    ON developer_execution_children (agent_id, task_id)
    WHERE task_id <> '';

CREATE TABLE IF NOT EXISTS developer_child_events (
    child_id UUID NOT NULL REFERENCES developer_execution_children(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS developer_child_messages (
    id UUID PRIMARY KEY,
    child_id UUID NOT NULL REFERENCES developer_execution_children(id) ON DELETE CASCADE,
    message_id TEXT NOT NULL UNIQUE,
    ...
    CONSTRAINT developer_child_messages_state_check CHECK (
