...
from niuu.collaboration import Participant, RoomState
from skuld.broker import ConversationTurn
from skuld.config import RoomConfig, SkuldSettings


class TestParticipant:
    """Tests for the shared Participant dataclass."""
    ...
    def test_equality(self):
        p1 = Participant(peer_id="x", persona="X", color="p4", participant_type="human")
        p2 = Participant(peer_id="x", persona="X", color="p4", participant_type="human")
        assert p1 == p2

    def test_inequality(self):
        p1 = Participant(peer_id="x", persona="X", color="p4", participant_type="human")
        p2 = Participant(peer_id="y", persona="Y", color="p4", participant_type="human")
        assert p1 != p2
...
class TestRoomState:
    ...
    def test_room_with_participants(self):
        p1 = Participant(peer_id="u1", persona="Alice", color="p1", participant_type="human")
        p2 = Participant(peer_id="a1", persona="Bot", color="p2", participant_type="ravn")
