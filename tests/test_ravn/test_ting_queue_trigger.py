"""Unit + integration tests for TingQueueTrigger (NIU-618).

Covers:
1. Trigger polling: mock Ting API responses, verify ready issues → AgentTasks
2. Dispatcher state respect: running=False/auto_continue=False/max_concurrent → no enqueue
3. Deduplication: same run returned twice → only one AgentTask enqueued
...
from ravn.adapters.triggers.ting_queue import (
    _DISPATCHER_PATH,
    _QUEUE_PATH,
    TingQueueTrigger,
...
def _make_trigger(
    ...
    return TingQueueTrigger(
        ting_base_url=base_url,
...
def _queue_item(
    ...
    title: str = "Add TingQueueTrigger",
    description: str = "Implement poll-based trigger for run dispatch.",
    saga_id: str = "saga-1",
    saga_name: str = "Vaka",
...
async def _collect_enqueued(
    trigger: TingQueueTrigger,
    items: list[dict] | None = None,
    dispatcher: dict | None = None,
) -> list[AgentTask]:
    """Run _poll_once with mocked Ting endpoints, collect enqueued tasks."""
...
class TestTriggerPolling:
    ...
    @pytest.mark.asyncio
    async def test_task_id_prefixed_ting_run(self) -> None:
        trigger = _make_trigger()
        items = [_queue_item(identifier="NIU-618")]
        enqueued = await _collect_enqueued(trigger, items=items)
        assert enqueued[0].task_id.startswith("ting_run_")

    @pytest.mark.asyncio
    async def test_title_from_queue_item(self) -> None:
        trigger = _make_trigger()
        items = [_queue_item(title="Add TingQueueTrigger")]
        enqueued = await _collect_enqueued(trigger, items=items)
        assert enqueued[0].title == "Add TingQueueTrigger"
    ...
    @pytest.mark.asyncio
    async def test_persona_is_run_executor(self) -> None:
        trigger = _make_trigger()
        items = [_queue_item()]
        enqueued = await _collect_enqueued(trigger, items=items)
        assert enqueued[0].persona == "run-executor"
    ...
    async def test_triggered_by_contains_saga_name(self) -> None:
        ...
        enqueued = await _collect_enqueued(trigger, items=items)
        assert enqueued[0].triggered_by == "ting_queue:Vaka"
    ...
    async def test_initiative_context_contains_title(self) -> None:
        ...
        items = [_queue_item(title="Add TingQueueTrigger")]
        enqueued = await _collect_enqueued(trigger, items=items)
        assert "Add TingQueueTrigger" in enqueued[0].initiative_context
    ...
    async def test_multiple_items_all_enqueued_within_capacity(self) -> None:
        ...
        dispatcher = _dispatcher_state(max_concurrent_runs=5)
...
class TestDispatcherStateRespect:
    ...
    async def test_max_concurrent_reached_no_enqueue(self) -> None:
        """When in-flight runs still appear in queue, no new runs are enqueued."""
        trigger = _make_trigger()
        ...
        dispatcher = _dispatcher_state(max_concurrent_runs=3)
        enqueued = await _collect_enqueued(trigger, items=items, dispatcher=dispatcher)
        assert enqueued == []

    @pytest.mark.asyncio
    async def test_one_below_max_enqueues_one(self) -> None:
        """With one slot free, exactly one new run is enqueued."""
        trigger = _make_trigger()
...
class TestDeduplication:
    @pytest.mark.asyncio
    async def test_same_run_enqueued_once_across_polls(self) -> None:
        trigger = _make_trigger()
    ...
    @pytest.mark.asyncio
    async def test_completed_run_cleared_from_tracking(self) -> None:
        """When a run leaves the queue (dispatched), its ID is removed from tracking."""
        trigger = _make_trigger()
    ...
    @pytest.mark.asyncio
    async def test_new_run_enqueued_after_previous_completes(self) -> None:
        """After first run leaves the queue a new run can be enqueued."""
        trigger = _make_trigger()
        item_first = _queue_item(issue_id="id-first", identifier="NIU-100")
        item_second = _queue_item(issue_id="id-second", identifier="NIU-200")
        dispatcher = _dispatcher_state(max_concurrent_runs=1)
        enqueued: list[AgentTask] = []
        ...
        assert len(enqueued) == 2
        assert enqueued[1].task_id.startswith("ting_run_NIU-200")

    @pytest.mark.asyncio
    async def test_capacity_limits_enqueue_count(self) -> None:
        """Only up to max_concurrent_runs items enqueued per poll."""
        trigger = _make_trigger()
        items = [_queue_item(issue_id=f"id-{i}", identifier=f"NIU-{i}") for i in range(5)]
        dispatcher = _dispatcher_state(max_concurrent_runs=2)
        enqueued = await _collect_enqueued(trigger, items=items, dispatcher=dispatcher)
        assert len(enqueued) == 2
...
class TestDriveLoopIntegration:
    @pytest.mark.asyncio
    async def test_tasks_appear_in_queue_when_ting_returns_ready_items(self) -> None:
        ...
        assert collected[0].persona == "run-executor"
...
class TestDynamicAdapterLoading:
    def test_trigger_importable_via_dotted_path(self) -> None:
        """TingQueueTrigger can be imported from the config-specified dotted path."""
        module = importlib.import_module("ravn.adapters.triggers.ting_queue")
        cls = getattr(module, "TingQueueTrigger")
        assert cls is TingQueueTrigger

    def test_trigger_instantiable_from_kwargs(self) -> None:
        """TingQueueTrigger can be instantiated from plain kwargs (as config wiring does)."""
        kwargs = {
            "ting_base_url": "http://ting:8080",
            "poll_interval_s": 30.0,
            "pat_token": "secret",
        }
        trigger = TingQueueTrigger(**kwargs)
        assert trigger.name == "ting_queue"
        assert trigger._ting_base_url == "http://ting:8080"
    ...
    def test_trailing_slash_stripped_from_base_url(self) -> None:
        ...
        assert not trigger._ting_base_url.endswith("/")
...
class TestBuildInitiativeContext:
    ...
    def test_contains_repos(self) -> None:
        item = _queue_item(repos=["volundr", "ting"])
        ctx = _build_initiative_context(item)
