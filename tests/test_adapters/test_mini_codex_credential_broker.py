"""Tests for the explicit mini-mode Codex credential broker."""
...
from volundr.adapters.outbound.mini_codex_credential_broker import (
...
def _broker(store: _Store, lock: _RefreshLock, oauth_client) -> MiniModeCodexCredentialBroker:
    return MiniModeCodexCredentialBroker(
        mini_mode=True,
...
@pytest.mark.parametrize("mini_mode,locked", [(False, True), (True, False)])
def test_mini_broker_rejects_production_or_unlocked_configuration(mini_mode, locked):
    with pytest.raises(ValueError, match="mini mode and a refresh lock"):
        MiniModeCodexCredentialBroker(
            credential_store=_Store(_auth_document(access_token=_access_token(expires_in=3600))),
            refresh_lock=_RefreshLock() if locked else None,
            mini_mode=mini_mode,
        )


@pytest.mark.parametrize(
    "metadata", [{"tenant_id": "other"}, {"renewal_owner": "openbao-oauthapp"}]
)
async def test_mini_broker_rejects_other_tenants_and_managed_grants(metadata):
    store = _Store(_auth_document(access_token=_access_token(expires_in=-60)))
    store.metadata = metadata
    oauth = _OAuthClient({})
    with pytest.raises(CodexCredentialBrokerError):
        await _broker(store, _RefreshLock(), oauth).get_tokens(
            owner_id="owner-1",
            tenant_id="tenant-1",
            credential_name="codex-main",
            credential_field="auth.json",
        )
    assert not oauth.posts
    assert not store.stores


def test_composition_enforces_deployment_mode():
    from volundr.composition_builders import _create_codex_credential_broker
    from volundr.config import DynamicAdapterConfig

    config = DynamicAdapterConfig(
        adapter="volundr.adapters.outbound.mini_codex_credential_broker.MiniModeCodexCredentialBroker"
    )
    settings = SimpleNamespace(
        codex_credential_broker=config, local_mounts=SimpleNamespace(mini_mode=False)
    )
    store = _Store(_auth_document(access_token=_access_token(expires_in=3600)))
    with pytest.raises(ValueError, match="mini mode"):
        _create_codex_credential_broker(
            settings, credential_store=store, refresh_lock=_RefreshLock()
        )
    settings.local_mounts.mini_mode = True
    assert isinstance(
        _create_codex_credential_broker(
            settings, credential_store=store, refresh_lock=_RefreshLock()
        ),
