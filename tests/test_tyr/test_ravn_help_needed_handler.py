...
from tyr.adapters.ravn_help_needed_handler import RavnHelpNeededHandler
from tyr.domain.models import RaidStatus

from .test_raids_api import StatefulMockTracker, _make_raid, _make_saga
...
async def test_handler_persists_help_needed_and_emits_notification_event() -> None:
    ...
    raid = _make_raid(status=RaidStatus.ESCALATED, session_id="session-council-1")
    tracker.raids[raid.id] = raid
    tracker.saga = _make_saga()
    tracker.get_raid_by_session = AsyncMock(return_value=raid)  # type: ignore[method-assign]
    ...
    messages = tracker.messages[raid.id]
    ...
    assert emitted.event == "raid.feedback_requested"
    assert emitted.owner_id == "dev-user"
    assert emitted.data["raid_id"] == str(raid.id)
    assert emitted.data["summary"] == "Need your call on the final recommendation."
    assert emitted.data["ui_path"] == f"/tyr/sagas/{tracker.saga.id}"
...
async def test_handler_dedupes_duplicate_open_requests() -> None:
    tracker = StatefulMockTracker()
    raid = _make_raid(status=RaidStatus.ESCALATED, session_id="session-council-2")
    tracker.raids[raid.id] = raid
    tracker.saga = _make_saga()
    tracker.get_raid_by_session = AsyncMock(return_value=raid)  # type: ignore[method-assign]
    event_bus = InMemoryEventBus()
    ...
    assert len(tracker.messages[raid.id]) == 1
