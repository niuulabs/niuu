...
from tyr.adapters.webhook_notification import WebhookNotificationAdapter
from tyr.ports.notification_channel import Notification, NotificationUrgency


def _notification(urgency: NotificationUrgency = NotificationUrgency.HIGH) -> Notification:
    return Notification(
        title="Raid ready for review",
        body="Raid NIU-100 is ready for review.",
        urgency=urgency,
        owner_id="dev-user",
        event_type="raid.state_changed",
...
async def test_send_posts_json_to_url() -> None:
    ...
    assert body["title"] == "Raid ready for review"
    assert body["event_type"] == "raid.state_changed"
