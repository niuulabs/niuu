...
from ting.api.developer_executions import _assert_delivery_claims
...
def _execution(**changes) -> DeveloperExecution:
    now = datetime.now(UTC)
    value = DeveloperExecution(
        id=uuid4(),
...
def _proposal(execution: DeveloperExecution, key: str, dependencies=()) -> WorkstreamProposal:
...
def _bound_work_order(execution: DeveloperExecution, key: str = "api") -> WorkstreamProposal:
...
def test_expansion_requires_expected_outputs_and_exact_nested_workspace() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="missing: expectedOutputs"):
        validate_expansion(
            execution,
            coordinator_id=execution.policy.coordinator_id,
            generation=1,
    ...
    with pytest.raises(DeveloperExecutionError, match="input workspace differs"):
        validate_expansion(
            execution,
            coordinator_id=execution.policy.coordinator_id,
            generation=1,
...
def test_schema_validation_rejects_missing_unknown_and_wrong_types() -> None:
    schema = _schema()
    with pytest.raises(DeveloperExecutionError, match="missing"):
        validate_json_instance({}, schema, field="input")
    with pytest.raises(DeveloperExecutionError, match="unknown fields"):
        validate_json_instance({"attemptId": "x", "extra": True}, schema, field="input")
    with pytest.raises(DeveloperExecutionError, match="must be string"):
...
def _request_with_auth(execution: DeveloperExecution, child: ChildExecution) -> Request:
...
class MemoryRepository(DeveloperExecutionRepository):
    def __init__(self, execution: DeveloperExecution) -> None:
...
async def test_message_is_idempotent_and_replies_to_blocked_child() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="metadata.requestId is required"):
    ...
    with pytest.raises(DeveloperExecutionError, match="exact outstanding question"):
        await service.message(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
...
async def test_gate_reply_requires_exact_outstanding_gate() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="metadata.gateId is required"):
        await service.message(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
    ...
    with pytest.raises(DeveloperExecutionError, match="metadata.gateDecision is required"):
        await service.message(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
    ...
    with pytest.raises(DeveloperExecutionError, match="not both"):
        await service.message(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
...
async def test_retry_does_not_create_attempt_when_blocked_writer_remains_active() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="still active"):
        await service.retry(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
...
async def test_retry_does_not_create_attempt_when_a2a_cancel_fails() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="could not be stopped"):
        await service.retry(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
...
async def test_retry_routes_only_to_exact_current_generation_attempt() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="no longer current"):
        await service.retry(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
...
async def test_retry_and_message_reject_invalid_targets() -> None:
    ...
    repository = MemoryRepository(execution)
    service = _service(repository)
    with pytest.raises(DeveloperExecutionError, match="unknown child"):
        await service.retry(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
    ...
    with pytest.raises(DeveloperExecutionError, match="required"):
        await service.message(
            execution.id,
            owner_id=execution.owner_id,
            tenant_id=execution.tenant_id,
