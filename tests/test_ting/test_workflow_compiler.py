"""Tests for Ting workflow DAG compilation."""
...
from ting.domain.workflow_compiler import compile_workflow_definition, compile_workflow_graph
...
class TestWorkflowCompiler:
    ...
    def test_reports_conditional_nodes_as_not_yet_executable(self) -> None:
        ...
        assert result.errors == ["Conditional nodes are not yet executable in Ting runtime: Route."]
    ...
    def test_reports_branching_as_not_yet_executable(self) -> None:
        ...
        assert result.definition_yaml is None
        assert result.errors == [
            "Node 'Review' fans out to multiple paths, which Ting cannot execute yet.",
    ...
    def test_reports_cycles_non_runtime_nodes_and_multiple_inbound_edges(self) -> None:
        ...
        assert cycle.errors == ["Workflow graph contains a cycle at node 'Start'."]
        ...
        assert unsupported.errors == ["Node 'Comment' is not executable in Ting runtime."]
        ...
        assert inbound.errors == [
            "Node 'Three' has multiple inbound edges, which Ting cannot execute yet.",
    ...
    def test_reports_stage_validation_errors(self) -> None:
        ...
        assert missing_member_id.errors == ["Stage 'Review' contains a member without personaId."]
