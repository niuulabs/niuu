"""Shared in-memory stubs reused across Ting test modules."""
...
class InMemorySagaRepository(SagaRepository):
    ...
    async def save_run(self, run: Run, *, conn: Any = None) -> None:
    ...
    async def get_runs_by_phase(self, phase_id: UUID) -> list[Run]:
        return [r for r in self.runs.values() if r.phase_id == phase_id]
...
_DEFAULT_TRACKER_ID = "run-tracker-001"
_DEFAULT_SESSION = "sess-ravn-001"


class StubTracker(TrackerPort):
    """Minimal in-memory tracker for Ting unit tests."""

    def __init__(self, run: Run | None = None) -> None:
        self._run = run
        self._runs_by_session: dict[str, Run] = {}
        if run is not None:
            self._runs_by_id = {run.tracker_id: run}
            if run.session_id:
    ...
    def runs(self) -> dict[str, Run]:
        """Public alias for _runs_by_id used in tests."""
        return self._runs_by_id
    ...
    async def update_run_progress(
        ...
        *,
        ...
        updated = Run(
            ...
            review_round=review_round if review_round is not None else run.review_round,
        )
        self._runs_by_id[tracker_id] = updated
        if updated.session_id:
            self._runs_by_session[updated.session_id] = updated
        return updated
    ...
    async def list_runs_by_status(self, status: RunStatus) -> list[Run]:
        return [r for r in self._runs_by_id.values() if r.status == status]
...
class StubGit:
    """Minimal in-memory git stub for Ting unit tests."""
...
def make_run(
    *,
    ...
    """Build a minimal Run for use in tests."""
    return Run(
        id=uuid4(),
        phase_id=uuid4(),
