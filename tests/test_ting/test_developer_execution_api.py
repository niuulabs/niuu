...
import ting.api.developer_executions as execution_api
...
from tests.test_ting.test_developer_execution import (
    MemoryRepository,
    RecordingGateway,
    _execution,
    _proposal,
...
def _token(execution, *, forge_session_id: str | None = None) -> str:
    ...
    return jwt.encode(
        ...
        "test-key-for-developer-execution-32-bytes",
        algorithm="HS256",
    )
...
def _settings(*, anonymous: bool = False):
    return SimpleNamespace(
        ...
        developer_execution=SimpleNamespace(
            lease_seconds=60.0,
...
def test_launch_reserves_frozen_snapshot_before_parent_session(monkeypatch) -> None:
    ...
    async def launch(**kwargs):
        ...
        developer = kwargs["launch"].provenance["developer_execution"]
...
def test_replayed_launch_context_uses_reserved_identity_and_excludes_token(monkeypatch) -> None:
    ...
    async def launch(**kwargs):
        ...
        developer = context["developer_execution"]
        assert developer == {
        ...
        assert "auth_token" not in kwargs["launch"].provenance["developer_execution"]
    ...
    assert captured["launch"].context["developer_execution"]["execution_id"] == str(reserved_id)
...
def test_execution_owner_isolation_and_list_shape() -> None:
    ...
    listed = client.get("/api/v1/ting/developer-executions", headers=_headers())
    foreign = client.get(
        f"/api/v1/ting/developer-executions/{execution.id}",
...
def test_execution_trace_uses_frozen_graph_and_public_outcomes_only() -> None:
    ...
    response = client.get(
        f"/api/v1/ting/developer-executions/{execution.id}/trace?after=3&limit=4",
        headers=_headers(),
    )
    ...
    foreign = client.get(
        f"/api/v1/ting/developer-executions/{execution.id}/trace",
        headers=_headers(owner="foreign-owner"),
    )
    assert foreign.status_code == 404
...
def test_execution_trace_child_uses_persisted_campaign_session_and_child_graph() -> None:
    ...
    response = TestClient(app).get(
        f"/api/v1/ting/developer-executions/{execution.id}/trace?childId={child.id}",
        headers=_headers(),
    )
...
def test_operator_mutations_bind_scoped_workloads_to_exact_parent_session() -> None:
    ...
    app.include_router(create_developer_executions_router())
    app.dependency_overrides[resolve_developer_execution_repo] = lambda: repository
    app.dependency_overrides[resolve_developer_execution_service] = lambda: service
    client = TestClient(app)
    url = f"/api/v1/ting/developer-executions/{execution.id}"
...
def test_evidence_reports_rejection_without_a_blocking_reason() -> None:
    ...
    response = TestClient(app).get(
        f"/api/v1/ting/developer-executions/{execution.id}/evidence",
        headers=_headers(),
    )
...
def test_record_integration_persists_only_forge_inspected_candidate() -> None:
    ...
    app.state.settings = SimpleNamespace(
        ...
        developer_execution=SimpleNamespace(integration_policy_id="developer-integration"),
    )
    app.include_router(create_developer_executions_router())
    app.dependency_overrides[resolve_developer_execution_repo] = lambda: repository
    app.dependency_overrides[resolve_volundr_factory] = lambda: Factory()
    ...
    response = TestClient(app).post(
        f"/api/v1/ting/developer-executions/{execution.id}/integration-candidate",
        headers={
...
def test_conditional_merge_authorization_requires_stored_review_and_exact_policy() -> None:
    ...
    accepted = client.post(
        f"/api/v1/ting/developer-executions/{execution.id}/delivery-authorizations",
        headers=headers,
        json=body,
    )
...
def test_publish_branch_authorization_requires_exact_allocated_non_base_branch() -> None:
    ...
    url = f"/api/v1/ting/developer-executions/{execution.id}/delivery-authorizations"
...
def test_delivery_wait_route_binds_owner_and_exact_coordinator_before_registration():
    ...
    app = FastAPI()
    app.state.settings = _settings()
    app.state.developer_delivery_wait_service = service
    app.include_router(create_developer_executions_router())
    app.dependency_overrides[resolve_developer_execution_repo] = lambda: repository
    client = TestClient(app)
    url = f"/api/v1/ting/developer-executions/{execution.id}/delivery-waits"
    payload = {
        "mode": "checks",
        "repository": execution.repository,
        "reviewNumber": 8,
...
def test_delivery_wait_route_reports_configuration_and_contract_failures():
    ...
    service = SimpleNamespace(
        request_wait=AsyncMock(
            side_effect=execution_api.DeveloperExecutionError(
