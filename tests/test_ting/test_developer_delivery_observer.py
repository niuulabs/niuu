...
from ting.adapters.developer_delivery_observer import ForgeDeveloperDeliveryObserver
...
def _setup(*, mode="checks", conclusions=(CheckConclusion.PASSING,)):
    ...
    observer = ForgeDeveloperDeliveryObserver(
...
async def test_required_check_status_and_exact_owner_binding(conclusion, status):
    ...
    assert claims["developer_execution_id"] == str(execution.id)
    assert claims["parent_node_id"] == execution.parent_node_id
    assert claims["scopes"] == ["ting:developer:coordinate"]
