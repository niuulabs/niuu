...
from ting.domain.pipeline_executor import TemplateAwarePipelineExecutor
from ting.ports.volundr import SpawnRequest, VolundrSession
...
async def _simulate_session_end(
    *,
    ...
    """Simulate a run completing: store outcome + publish Sleipnir event."""
    await executor.receive_outcome(run_id=run_id, outcome=outcome)
...
async def test_full_review_pipeline(tmp_path: Path) -> None:
    ...
    # --- 2. Phase 1 should be ACTIVE with 2 runs running ----------------------
    phase1 = all_phases[0]
    assert phase1.status == PhaseStatus.ACTIVE
    assert phase1.name == "parallel-review"

    phase1_runs = await repo.get_runs_by_phase(phase1.id)
    assert len(phase1_runs) == 2
    assert all(r.status == RunStatus.RUNNING for r in phase1_runs)

    reviewer_run = next(r for r in phase1_runs if "review" in r.name.lower())
    security_run = next(
        r for r in phase1_runs if "security" in r.name.lower() or "audit" in r.name.lower()
    )
    ...
    # --- 6. Phase 2 (QA) should be ACTIVE with 1 run --------------------------
    phase2 = all_phases[1]
    ...
    phase2_runs = await repo.get_runs_by_phase(phase2.id)
    assert len(phase2_runs) == 1
    qa_run = phase2_runs[0]
    assert qa_run.status == RunStatus.RUNNING
...
async def test_dynamic_pipeline_via_api() -> None:
    ...
    # Phase 1 run should be RUNNING (auto_start=True dispatched it)
    phases = await repo.get_phases_by_saga(saga_id)
    phase1_runs = await repo.get_runs_by_phase(phases[0].id)
    assert len(phase1_runs) == 1
    assert phase1_runs[0].status == RunStatus.RUNNING

    # Simulate phase 1 completing → gate phase becomes GATED
    await executor.receive_outcome(
        run_id=phase1_runs[0].id,
...
async def test_pipeline_fails_on_fan_in_rejection() -> None:
    ...
    # Phase 1 should be COMPLETE (all runs terminal) but saga FAILED
    phase1_final = await repo.get_phase(phase1.id)
    assert phase1_final.status == PhaseStatus.COMPLETE
...
async def test_approve_gate_advances_to_next_pending_phase() -> None:
    ...
    phase1_runs = await repo.get_runs_by_phase(phases[0].id)
    await executor.receive_outcome(run_id=phase1_runs[0].id, outcome={"verdict": "pass"})
