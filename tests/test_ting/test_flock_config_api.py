...
from ting.config import AuthConfig, FlockConfig, PersonaOverride, Settings
...
def _make_app(flock: FlockConfig | None = None) -> FastAPI:
    ...
    # Always set flock explicitly so local ting.yaml doesn't leak into tests
...
class TestPatchFlockConfig:
    def test_toggle_flock_enabled(self, client: TestClient) -> None:
        resp = client.patch("/api/v1/ting/flock/config", json={"flock_enabled": True})
        assert resp.status_code == 200
        data = resp.json()
        assert data["flock_enabled"] is True

    def test_toggle_flock_disabled(self, client_enabled: TestClient) -> None:
        resp = client_enabled.patch("/api/v1/ting/flock/config", json={"flock_enabled": False})
        assert resp.status_code == 200
        data = resp.json()
        assert data["flock_enabled"] is False
    ...
    def test_empty_patch_returns_current(self, client_enabled: TestClient) -> None:
        resp = client_enabled.patch("/api/v1/ting/flock/config", json={})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_runtime_flock_config_does_not_change_operator_settings(
        ...
        resp = client.patch("/api/v1/ting/flock/config", json={"flock_enabled": True})
        settings_resp = client.get("/api/v1/ting/settings/flock")
