...
from tests.test_ting.test_developer_execution import MemoryRepository, _execution
...
def _context():
    ...
    event = ActivityEvent(
        ...
        state="active",
        metadata={"developer_review": review},
...
async def test_integration_projector_requires_literal_validation_before_signing(valid) -> None:
    repository, _, projector, event = _context()
    event.metadata["developer_review"]["valid"] = valid

    with pytest.raises(DeveloperExecutionError, match="lineage is invalid"):
...
async def test_real_integration_persona_contract_projects_exact_allocation_id(tmp_path) -> None:
    ...
    assert live["developer_review"]["attemptId"] == "integration-attempt-1"
...
async def test_rejects_review_for_stale_integration_candidate() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="lineage"):
        await projector.handle_activity(event, repository.execution.owner_id)
...
async def test_rejects_review_before_trusted_candidate_inspection() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="before trusted candidate"):
        await projector.handle_activity(event, repository.execution.owner_id)
...
async def test_rejects_foreign_integration_review_lineage(mutation: str) -> None:
    repository, _, projector, event = _context()
    review = event.metadata["developer_review"]
    if mutation == "persona":
...
async def test_ignores_activity_without_integration_review() -> None:
    ...
    event.metadata["developer_review"] = {"scope": "workstream"}
    assert not await projector.handle_activity(event, repository.execution.owner_id)
...
async def test_rejects_unbound_session_and_malformed_findings() -> None:
    ...
    with pytest.raises(DeveloperExecutionError, match="not bound"):
        await projector.handle_activity(event, "another-owner")

    event.metadata["developer_review"]["findings"] = ["not-an-object"]
    with pytest.raises(DeveloperExecutionError, match="findings are malformed"):
        await projector.handle_activity(event, repository.execution.owner_id)
...
async def test_rejects_invalid_verdict_and_malformed_receipt() -> None:
    repository, _, projector, event = _context()
    event.metadata["developer_review"]["verdict"] = "approve-ish"
    with pytest.raises(DeveloperExecutionError, match="verdict is invalid"):
        await projector.handle_activity(event, repository.execution.owner_id)

    event.metadata["developer_review"]["verdict"] = "pass"
    event.metadata["developer_review"]["summary"] = "x" * 70_000
    with pytest.raises(DeveloperExecutionError, match="receipt is invalid"):
        await projector.handle_activity(event, repository.execution.owner_id)
