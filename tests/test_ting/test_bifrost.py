...
from ting.adapters.anthropic_client import ANTHROPIC_API_VERSION
from ting.adapters.bifrost import (
    DECOMPOSITION_PROMPT,
    BifrostAdapter,
    DecompositionError,
)
from ting.adapters.bifrost_publisher import BifrostPublisher
from ting.domain.models import RunSpec, SagaStructure
from ting.domain.validation import ValidationError, parse_and_validate, validate_run
...
class TestParseAndValidate:
    def test_valid_response(self) -> None:
        ...
        run = result.phases[0].runs[0]
        assert run.name == "Add user model"
        assert run.confidence == 0.85
        assert run.estimate_hours == 3.0
    ...
    def test_phase_missing_name(self) -> None:
        data = {"name": "S", "phases": [{"runs": [VALID_RUN]}]}
        with pytest.raises(ValidationError, match="Phase 0.*'name'"):
            parse_and_validate(json.dumps(data))

    def test_phase_empty_runs(self) -> None:
        data = {"name": "S", "phases": [{"name": "P1", "runs": []}]}
        with pytest.raises(ValidationError, match="non-empty list"):
            parse_and_validate(json.dumps(data))

    def test_multiple_phases_and_runs(self) -> None:
        run2 = {**VALID_RUN, "name": "Second run"}
        data = {
            "name": "Multi",
            "phases": [
                {"name": "P1", "runs": [VALID_RUN]},
                {"name": "P2", "runs": [run2]},
            ],
        }
        result = parse_and_validate(json.dumps(data))
        assert len(result.phases) == 2
        assert result.phases[1].runs[0].name == "Second run"

    def test_integer_estimate_accepted(self) -> None:
        run = {**VALID_RUN, "estimate_hours": 4}
        data = {"name": "S", "phases": [{"name": "P", "runs": [run]}]}
        result = parse_and_validate(json.dumps(data))
        assert result.phases[0].runs[0].estimate_hours == 4.0

    def test_integer_confidence_accepted(self) -> None:
        run = {**VALID_RUN, "confidence": 1}
        data = {"name": "S", "phases": [{"name": "P", "runs": [run]}]}
        result = parse_and_validate(json.dumps(data))
        assert result.phases[0].runs[0].confidence == 1.0

    def test_custom_estimate_bounds(self) -> None:
        run = {**VALID_RUN, "estimate_hours": 1.0}
        data = {"name": "S", "phases": [{"name": "P", "runs": [run]}]}
        # Lenient validation clamps instead of rejecting
        result = parse_and_validate(json.dumps(data))
        assert result.phases[0].runs[0].estimate_hours == 1.0
            ...
            return_value=httpx.Response(
                ...
                json={
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": ' "phases": [{"name": "P", "runs": [',
                        },
                        {
                            "type": "text",
                            "text": json.dumps(VALID_RUN) + "]}]}",
                        },
                    ]
                },
            )
