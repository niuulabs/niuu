"""Tests for ting.domain.pipeline_executor — PipelineExecutor and fan-in logic."""
...
from ting.domain.models import PhaseStatus, RunStatus, SagaStatus
from ting.domain.pipeline_executor import (
...
class TestCreateFromYaml:
    ...
    @pytest.mark.asyncio
    async def test_creates_phases_and_runs(self):
        executor, repo, _ = _make_executor()
        saga = await executor.create_from_yaml(_SINGLE_STAGE_PIPELINE)
        phases = await repo.get_phases_by_saga(saga.id)
        assert len(phases) == 1
        runs = await repo.get_runs_by_phase(phases[0].id)
        assert len(runs) == 1

    @pytest.mark.asyncio
    async def test_parallel_stage_creates_two_runs(self):
        executor, repo, _ = _make_executor()
    ...
    @pytest.mark.asyncio
    async def test_auto_start_dispatches_first_phase_runs(self):
    ...
    @pytest.mark.asyncio
    async def test_parallel_auto_start_dispatches_both_runs(self):
        volundr = StubVolundrPort()
        executor, repo, _ = _make_executor(volundr=volundr)
...
class TestReceiveOutcome:
    ...
    async def test_receive_outcome_stores_structured_outcome(self):
        ...
        await executor.receive_outcome(run_id=run.id, outcome=outcome)

        updated_run = await repo.get_run(run.id)
        assert updated_run is not None
        assert updated_run.structured_outcome == outcome
    ...
    async def test_pass_verdict_sets_merged_status(self):
        ...
        await executor.receive_outcome(run_id=runs[0].id, outcome={"verdict": "pass"})

        updated = await repo.get_run(runs[0].id)
    ...
    async def test_fail_verdict_sets_failed_status(self):
        ...
        await executor.receive_outcome(run_id=runs[0].id, outcome={"verdict": "fail"})

        updated = await repo.get_run(runs[0].id)
        assert updated.status == RunStatus.FAILED

    @pytest.mark.asyncio
    async def test_unknown_run_is_noop(self):
        executor, _, _ = _make_executor()
        # Should not raise
        await executor.receive_outcome(run_id=uuid.uuid4(), outcome={"verdict": "pass"})
...
class TestParallelFanIn:
    @pytest.mark.asyncio
    async def test_both_runs_must_complete_before_phase_advances(self):
        """Phase 1 has 2 parallel runs. Phase 2 should not start until both complete."""
        volundr = StubVolundrPort()
        ...
        phases = await repo.get_phases_by_saga(saga.id)
        phase1 = phases[0]
        runs = await repo.get_runs_by_phase(phase1.id)
        assert len(runs) == 2

        # Simulate first reviewer completing
        await executor.receive_outcome(run_id=runs[0].id, outcome={"verdict": "pass"})

        # Phase 1 should still be ACTIVE (second run not done yet)
        ...
        await executor.receive_outcome(
            run_id=runs[1].id, outcome={"verdict": "pass", "critical_findings": 0}
        )
    ...
    @pytest.mark.asyncio
    async def test_all_must_pass_fan_in_fails_saga_when_run_fails(self):
        """When fan_in=all_must_pass and a run fails, saga should fail."""
        volundr = StubVolundrPort()
        executor, repo, bus = _make_executor(volundr=volundr)
        saga = await executor.create_from_yaml(_PARALLEL_PIPELINE, auto_start=True)
        ...
        await executor.receive_outcome(run_id=runs[0].id, outcome={"verdict": "pass"})
        # Second run fails
        await executor.receive_outcome(run_id=runs[1].id, outcome={"verdict": "fail"})
...
class TestPipelineTemplateLoading:
    def test_load_review_template(self):
        from ting.domain.templates import BUNDLED_TEMPLATES_DIR, load_template
        ...
        assert len(template.phases[0].runs) == 2
...
class TestConditionBasedAdvancement:
    ...
    async def test_condition_failure_fails_saga(self):
        ...
        # Saga should be failed because fan-in failed (FAILED run → has_failed=True)
        updated_saga = await repo.get_saga(saga.id)
        assert updated_saga.status == SagaStatus.FAILED
    ...
    async def test_advance_phase_with_no_volundr_does_not_raise(self):
        ...
        # Mark run as RUNNING manually (skip auto_start since no volundr)
        from datetime import UTC, datetime

        from ting.domain.models import Run

        updated = Run(
            id=runs[0].id,
            phase_id=runs[0].phase_id,
            tracker_id=runs[0].tracker_id,
            name=runs[0].name,
            description=runs[0].description,
            acceptance_criteria=runs[0].acceptance_criteria,
            declared_files=runs[0].declared_files,
            estimate_hours=runs[0].estimate_hours,
            status=RunStatus.RUNNING,
            confidence=runs[0].confidence,
            session_id="mock-session",
            branch=runs[0].branch,
            chronicle_summary=runs[0].chronicle_summary,
            pr_url=runs[0].pr_url,
            pr_id=runs[0].pr_id,
            retry_count=runs[0].retry_count,
    ...
    async def test_condition_eval_error_fails_saga(self):
        ...
        from ting.domain.condition_evaluator import ConditionError
        ...
        with patch(
            "ting.domain.pipeline_executor.evaluate_condition",
    ...
    async def test_base_create_from_yaml_works(self):
        ...
        from ting.domain.pipeline_executor import PipelineExecutor
...
class TestContextInjectionE2E:
    ...
    async def test_stage2_receives_stage1_outcomes_in_prompt(self):
        ...
        # Phase 1 spawned 2 runs, phase 2 spawns 1 run → 3 total
    ...
    async def test_stage2_context_includes_stage1_verdicts(self):
        ...
        await executor.receive_outcome(
            run_id=runs[0].id, outcome={"verdict": "pass", "summary": "looks good"}
        )
        await executor.receive_outcome(
            run_id=runs[1].id, outcome={"verdict": "pass", "critical_findings": 0}
        )
...
class TestBuildFlockWorkloadConfig:
    ...
    def test_no_flow_name_returns_none(self):
        ...
        assert build_flock_workload_config("", tpl_run, StubFlockFlowProvider(), "prompt") is None
    ...
    def test_no_provider_returns_none(self):
        ...
        assert build_flock_workload_config("my-flow", tpl_run, None, "prompt") is None
    ...
    def test_unknown_flow_returns_none(self):
        ...
        assert build_flock_workload_config("no-such-flow", tpl_run, provider, "prompt") is None
    ...
    def test_flow_only_returns_flow_personas(self):
        ...
        config = build_flock_workload_config("code-review-flow", tpl_run, provider, "review it")
    ...
    def test_initiative_context_set_to_initial_prompt(self):
        ...
        config = build_flock_workload_config("code-review-flow", tpl_run, provider, "my prompt")
    ...
    def test_mimir_url_included_when_set(self):
        ...
        config = build_flock_workload_config("my-flow", tpl_run, provider, "p")
...
class TestFlockFlowPipelineDispatch:
    ...
    async def test_reviewer_override_applied_security_auditor_unchanged(self):
        ...
        # 2 runs dispatched (reviewer + security-auditor)
        assert len(volundr.spawned) == 2

        # The reviewer run dispatch: its workload_config should have reviewer with powerful alias
        ...
        # The security-auditor run dispatch: its workload_config should have auditor unchanged
    ...
    @pytest.mark.asyncio
    async def test_flow_only_dispatches_identical_workload_config_for_both_runs(self):
        """Flow-only: both runers in a parallel stage get identical flow personas."""
        flow = _make_flow()
        provider = StubFlockFlowProvider({"code-review-flow": flow})
        volundr = StubVolundrPort()
        executor, _, _ = _make_executor_with_flow(provider, volundr)
