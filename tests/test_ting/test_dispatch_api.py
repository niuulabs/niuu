...
from ting.config import DispatchConfig as DispatchCfg
...
class DispatchAliasTracker(MockTracker):
    ...
    async def get_run(self, tracker_id: str):
...
class TestBuildPrompt:
    ...
    def test_template_renders_placeholders(self):
        ...
        assert "niu-42" in prompt  # run_branch is lowercased identifier
...
class TestGetConfig:
    def test_returns_defaults(self, client: TestClient):
        resp = client.get("/api/v1/ting/dispatch/config")
...
class TestGetQueue:
    def test_returns_ready_issues(self, client: TestClient):
        resp = client.get("/api/v1/ting/dispatch/queue")
        assert resp.status_code == 200
        data = resp.json()
...
class TestDispatchAliases:
    ...
    def test_dispatch_single_run_alias(self) -> None:
        ...
        response = client.post("/api/v1/ting/dispatch/i-1", headers=_auth_headers())
    ...
    def test_dispatch_single_run_alias_allows_repo_less_sagas(self) -> None:
        tracker = DispatchAliasTracker()
        tracker._saga = Saga(
            **{
    ...
    def test_dispatch_batch_alias_reports_dispatches_and_failures(self) -> None:
        ...
        response = client.post(
            "/api/v1/ting/dispatch/batch",
            json={"run_ids": ["i-1", "missing"]},
        ...
        assert response.json() == {
            "dispatched": ["i-1"],
            "failed": [{"runId": "missing", "reason": "run issue not found in tracker"}],
        }
