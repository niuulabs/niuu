"""Tests for ting.adapters.event_trigger.EventTriggerAdapter."""
...
from ting.adapters.event_trigger import (
    EventTriggerAdapter,
    _TriggerRule,
    matches_filter,
)
...
class TestLoadTemplate:
    def test_loads_bundled_review_template(self, tmp_path):
        ...
        assert len(tpl.phases[0].runs) == 2  # parallel: reviewer + security-auditor
    ...
    def test_interpolation_replaces_event_placeholders(self, tmp_path):
        ...
        run = tpl.phases[0].runs[0]
        assert run.name == "Run for my-repo"
        assert "my-repo" in run.prompt
...
class TestEventTriggerAutoStart:
    ...
    async def test_matching_event_creates_phase_and_run(self, tmp_path):
        _write_minimal_template(tmp_path, "review")
        saga_repo = InMemorySagaRepository()
        bus = InProcessBus()
        adapter = _make_adapter(
    ...
    async def test_auto_start_updates_run_to_running(self, tmp_path):
        _write_minimal_template(tmp_path, "review")
        saga_repo = InMemorySagaRepository()
        bus = InProcessBus()
        adapter = _make_adapter(
    ...
    async def test_auto_start_emits_saga_created_and_run_state_changed(self, tmp_path):
        _write_minimal_template(tmp_path, "review")
...
class TestEventTriggerPendingApproval:
    async def test_auto_start_false_creates_pending_run(self, tmp_path):
        _write_minimal_template(tmp_path, "review")
        saga_repo = InMemorySagaRepository()
        bus = InProcessBus()
        adapter = _make_adapter(
    ...
    async def test_auto_start_false_emits_needs_approval(self, tmp_path):
        ...
        approval_events = [e for e in events if e.event == "run.needs_approval"]
...
class TestBuildEventTriggerAdapter:
    def test_build_from_config(self, tmp_path):
        from ting.adapters.event_trigger import build_event_trigger_adapter
        from ting.config import EventTriggerConfig, EventTriggerRule
    ...
    def test_build_with_default_templates_dir(self):
        from ting.adapters.event_trigger import build_event_trigger_adapter
        from ting.config import EventTriggerConfig
...
# ---------------------------------------------------------------------------
# NotificationService: run.needs_approval
# ---------------------------------------------------------------------------
...
class TestNotificationServiceNeedsApproval:
    async def test_needs_approval_emits_high_urgency_notification(self):
        ...
        assert notif.event_type == "run.needs_approval"
        assert "Review PR #99" in notif.body

    async def test_needs_approval_without_owner_id_produces_no_notification(self):
        from ting.domain.services.notification import NotificationService
        from ting.ports.channel_resolver import ChannelResolverPort
        from ting.ports.notification_channel import Notification, NotificationChannel
        ...
        await event_bus.emit(
            TingEvent(
                event="run.needs_approval",
                data={"run_id": "r-1", "run_name": "Something"},  # no owner_id
                owner_id="",
            )
        )
...
def _write_minimal_template(tmp_path: Path, name: str) -> None:
    """Write a minimal single-phase, single-run YAML template."""
              ...
              - name: Execute
                runs:
                  - name: "Do the thing in {event.repo}"
                    description: "Automated saga run"
                    acceptance_criteria:
                      - "Task completed"
                    declared_files: []
                    estimate_hours: 1.0
...
class TestTemplateValidation:
    ...
    def test_missing_run_persona_raises(self, tmp_path):
    ...
    def test_phase_without_runs_raises(self, tmp_path):
        (tmp_path / "bad.yaml").write_text(
            textwrap.dedent("""\
                name: "Bad template"
                feature_branch: main
...
class TestTemplateDataclasses:
    def test_template_run_has_persona_field(self, tmp_path):
        _write_minimal_template(tmp_path, "tpl")
        tpl = load_template("tpl", tmp_path, {"repo": "r"})
        assert tpl.phases[0].runs[0].persona == "executor"
...
class TestMultiPhaseDispatch:
    async def test_multi_phase_only_phase_1_runs_dispatched_initially(self, tmp_path):
        ...
        # Only the Phase 1 run should be spawned
        assert len(volundr.spawned) == 1
    ...
    async def test_multi_phase_phase_2_starts_pending(self, tmp_path):
        ...
        from ting.domain.models import PhaseStatus
    ...
    async def test_multi_phase_all_runs_start_pending(self, tmp_path):
        _write_two_phase_template(tmp_path, "two")
        saga_repo = InMemorySagaRepository()
        bus = InProcessBus()
        adapter = _make_adapter(
        ...
        # Phase 1 run: RUNNING (was dispatched); Phase 2 run: PENDING
        runs_by_name = {r.name: r for r in saga_repo.runs.values()}
        assert runs_by_name["Phase 1 run"].status == RunStatus.RUNNING
        assert runs_by_name["Phase 2 run"].status == RunStatus.PENDING
...
class TestAdvancePhase:
    async def test_advance_phase_dispatches_next_phase_runs(self, tmp_path):
        _write_two_phase_template(tmp_path, "two")
        volundr = StubVolundrPort()
        saga_repo = InMemorySagaRepository()
        bus = InProcessBus()
        ...
        assert volundr.spawned[1].name == "phase-2-run"
...
class TestPersonaPassedToSpawnRequest:
    ...
    async def test_empty_persona_sets_profile_to_none(self, tmp_path):
        ...
        # Instead test the None path by calling _spawn_run with empty persona directly.
        ...
        await adapter_obj._spawn_run(volundr, saga, phase, run, tpl_run)
...
class TestBundledShipRetroTemplates:
    ...
    def test_retro_template_loads(self):
        ...
        for phase in tpl.phases:
            for run in phase.runs:
                assert run.persona == "retro-analyst"
