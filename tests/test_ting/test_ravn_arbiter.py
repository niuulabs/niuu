...
Test harness from NIU-619 spec:
  ...
  2. Unit: context assembly — given run with CI fail + scope breach, verify signals present
  ...
  5. Integration: review round-trip — mock dispatcher returning canned outcome;
     ReviewEngine dispatches to arbiter → outcome parsed → run state transitions
"""
...
class TestContextAssembly:
    def test_context_includes_acceptance_criteria(self) -> None:
        """Context string must include the run's acceptance criteria."""
        ...
        context = engine._assemble_arbiter_context(run, pr_status, [], 0.3)
    ...
    def test_context_includes_scope_breach_ratio(self) -> None:
        ...
        engine, _, _, _ = _make_engine()
        run = _make_run(declared_files=["src/main.py"])
        changed = ["src/main.py", "src/extra1.py", "src/extra2.py", "src/extra3.py"]
        context = engine._assemble_arbiter_context(run, None, changed, 0.2)
    ...
    def test_context_includes_confidence_score(self) -> None:
        ...
        engine, _, _, _ = _make_engine()
        run = _make_run()
        context = engine._assemble_arbiter_context(run, None, [], 0.42)
        assert "0.420" in context

    def test_context_includes_tracker_id(self) -> None:
        engine, _, _, _ = _make_engine()
        run = _make_run()
        context = engine._assemble_arbiter_context(run, None, [], 0.5)
    ...
    def test_context_declared_files_listed(self) -> None:
        engine, _, _, _ = _make_engine()
        run = _make_run(declared_files=["src/foo.py", "src/bar.py"])
        context = engine._assemble_arbiter_context(run, None, [], 0.5)
    ...
    def test_context_marks_declared_vs_undeclared(self) -> None:
        ...
        context = engine._assemble_arbiter_context(run, None, changed, 0.5)
...
class TestOutcomeMapping:
    ...
    async def test_approve_verdict_auto_approves(self) -> None:
        ...
        git.changed_files[PR_ID] = list(run.declared_files)
    ...
    async def test_retry_verdict_retries(self) -> None:
        ...
        assert tracker.runs[run.tracker_id].status == RunStatus.PENDING
    ...
    async def test_retry_verdict_exhausted_escalates(self) -> None:
        ...
        engine, tracker, git, _ = _make_engine(config=config, ravn=ravn)
        run = _make_run(confidence=0.4, retry_count=3)
        tracker.runs[run.tracker_id] = run
        _passing_pr(git, PR_ID)
        ...
        assert tracker.runs[run.tracker_id].status == RunStatus.ESCALATED
...
class TestReviewArbiterRoundTrip:
    ...
    async def test_arbiter_approve_transitions_to_merged(self) -> None:
        """Full round-trip: arbiter 'approve' → run transitions to MERGED."""
    ...
    async def test_arbiter_retry_transitions_to_pending(self) -> None:
        """Full round-trip: arbiter 'retry' → run transitions to PENDING."""
        ...
        run = _make_run(confidence=0.6, retry_count=1)
        tracker.runs[run.tracker_id] = run
        _passing_pr(git, PR_ID)
    ...
    async def test_arbiter_context_passed_to_dispatcher(self) -> None:
        ...
        engine, tracker, git, _ = _make_engine(config=config, ravn=ravn)
        run = _make_run(confidence=0.4, declared_files=["src/core.py"])
        tracker.runs[run.tracker_id] = run
