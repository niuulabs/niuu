...
class TestDispatchIssues:
    ...
    @pytest.mark.asyncio
    async def test_updates_run_progress(
        self,
        saga_repo: MockSagaRepo,
        dispatcher_repo: MockDispatcherRepo,
    ):
        """Verify run progress is updated after spawn."""
        ...
        class TrackingTracker(MockTracker):
            ...
            async def update_run_progress(self, tracker_id: str, **kwargs):
                self.progress_calls.append({"tracker_id": tracker_id, **kwargs})
                return await super().update_run_progress(tracker_id, **kwargs)
...
class TestBuildSpawnRequestPersonaOverrides:
    ...
    def test_per_persona_overrides_forwarded(self):
        ...
        assert emitted[0] == {"name": "run-executor"}
    ...
    def test_info_log_emitted(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="ting.domain.services.dispatch_service"):
...
def _make_template_run(persona: str = "reviewer", prompt: str = "review it") -> TemplateRun:
    return TemplateRun(
        name=f"{persona}-run",
        description="",
        acceptance_criteria=[],
...
def _make_template_phase(runs: list[TemplateRun] | None = None) -> TemplatePhase:
    runs = runs or [_make_template_run()]
    return TemplatePhase(name="review", runs=runs, fan_in="all_must_pass")
...
def _make_run(phase_id, persona: str = "reviewer") -> Run:
    run_id = uuid4()
...
class TestDispatchTemplatePhaseFlockFlow:
    ...
    async def test_flock_flow_name_resolves_to_ravn_flock(self):
        ...
        await service._dispatch_template_phase(
            saga, phase, [run], tpl_phase, "owner-1", flock_flow_name="code-review-flow"
        )
    ...
    async def test_empty_flock_flow_name_is_solo_dispatch(self):
        ...
        tpl_phase = _make_template_phase()
        await service._dispatch_template_phase(
            saga, phase, [run], tpl_phase, "owner-1", flock_flow_name=""
        )
