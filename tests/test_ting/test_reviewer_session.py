...
ISSUES:
- Critical: ting imports from volundr directly
...
class TestBuildReviewerInitialPrompt:
    ...
    def test_prompt_includes_acceptance_criteria(self) -> None:
        run = _make_run(acceptance_criteria=["coverage >= 85%", "no lint warnings"])
        prompt = build_reviewer_initial_prompt(
            run=run,
            pr_status=None,
            changed_files=[],
...
class TestSpawnReviewer:
    ...
    async def test_spawn_includes_diff_summary_from_chronicle(self) -> None:
        service, volundr = _make_service()
        run = _make_run(chronicle_summary="Implemented auth flow")
    ...
    async def test_spawn_with_no_chronicle(self) -> None:
        service, volundr = _make_service()
        run = _make_run(chronicle_summary=None)
...
class TestSendFeedback:
    ...
    async def test_sends_feedback_with_issues(self) -> None:
        ...
        result = ReviewerResult(
            ...
            findings=["ting imports from volundr", "missing tests"],
            approved=False,
        )
        ...
        assert "ting imports from volundr" in msg
        assert "0.65" in msg
...
class _TrackerStub:
    ...
    async def update_run_progress(self, tracker_id: str, **kwargs) -> Run:
        run = self.runs[tracker_id]
        ...
        updated = Run(
            ...
            reviewer_session_id=kwargs.get("reviewer_session_id", run.reviewer_session_id),
            review_round=kwargs.get("review_round", run.review_round),
        )
    ...
    async def get_saga_for_run(self, tracker_id: str):
        return None

    async def get_phase_for_run(self, tracker_id: str):
        return None
...
class TestReviewEngineReviewerIntegration:
    ...
    async def test_engine_spawns_reviewer_when_enabled(self) -> None:
        ...
        from ting.domain.models import ConfidenceEventType
        ...
        events = tracker_stub.events.get(run.tracker_id, [])
        ...
        assert engine.get_reviewer_run("reviewer-1") == (run.tracker_id, OWNER_ID)
...
class TestHandleReviewerCompletion:
    ...
    async def test_completion_with_high_confidence_auto_approves(self) -> None:
        ...
        # After completion, the run should have been auto-approved (MERGED)
        updated_run = tracker_stub.runs[run.tracker_id]
    ...
    async def test_completion_with_low_confidence_escalates(self) -> None:
        ...
        assert updated_run.review_round == 1
        assert updated_run.status == RunStatus.REVIEW
    ...
    async def test_completion_sends_feedback_on_issues(self) -> None:
        ...
        assert engine.get_reviewer_run("reviewer-1") is not None
    ...
    async def test_completion_unparseable_chronicle_is_ignored(self) -> None:
        ...
        # Run status should remain REVIEW (unchanged)
        updated_run = tracker_stub.runs[run.tracker_id]
        assert updated_run.status == RunStatus.REVIEW

    @pytest.mark.asyncio
    async def test_get_reviewer_run_returns_none_for_unknown(self) -> None:
        from ting.adapters.memory_event_bus import InMemoryEventBus
        from ting.domain.services.review_engine import ReviewEngine

        engine = ReviewEngine(
            tracker_factory=_TrackerFactoryStub(_TrackerStub(_make_run())),
            volundr_factory=StubVolundrFactory(StubVolundr()),
            git=_FullGitStub(),
            review_config=_default_config(),
            event_bus=InMemoryEventBus(),
        )
        assert engine.get_reviewer_run("unknown-session") is None
