...
class _IntegrationConnection:
    ...
    async def fetchrow(self, query, *args):
        ...
        if query.startswith("SELECT * FROM workflow_executions WHERE id"):
            return dict(self.row)
        review_update = (
...
async def test_parent_launch_persists_snapshot_and_replays_same_digest() -> None:
    ...
    replayed, replay_created = await PostgresDeliveryExecutionRepository(
        replay_pool
...
async def test_parent_launch_rejects_missing_or_colliding_idempotency() -> None:
    execution = _execution()
    repository = PostgresDeliveryExecutionRepository(_LaunchPool(execution))
    ...
    with pytest.raises(ExecutionConflictError, match="different developer execution"):
        await PostgresDeliveryExecutionRepository(pool).reserve_parent_launch(attempted)
...
async def test_reserve_generation_rejects_changed_parent_state(mutation: str) -> None:
    ...
    with pytest.raises(ExecutionConflictError):
        await PostgresDeliveryExecutionRepository(_Pool(connection)).reserve_generation(
            execution,
            (child,),
            plan_revision=PLAN_REVISION,
        )
...
async def test_record_handle_is_lease_fenced_and_observation_is_replay_safe() -> None:
    ...
    replayed = await PostgresDeliveryExecutionRepository(
    ...
    with pytest.raises(ExecutionConflictError, match="fenced"):
        await PostgresDeliveryExecutionRepository(_Pool(fenced)).record_handle(
            child,
            ChildTaskHandle("agent-1", "task-1", "context-1"),
    ...
    retained = await PostgresDeliveryExecutionRepository(_Pool(raced)).record_handle(
        child,
...
async def test_blocker_observation_advances_parent_revision_only_when_changed() -> None:
    ...
    blocked = await PostgresDeliveryExecutionRepository(_Pool(first_connection)).record_observation(
        child, first
    )
    ...
    await PostgresDeliveryExecutionRepository(_Pool(repeated_connection)).record_observation(
        blocked,
        repeated,
    )
    ...
    await PostgresDeliveryExecutionRepository(_Pool(changed_connection)).record_observation(
        blocked,
        changed,
    )
...
async def test_observation_cannot_overwrite_terminal_or_newer_remote_state() -> None:
    ...
    kept = await PostgresDeliveryExecutionRepository(_Pool(connection)).record_observation(
        terminal,
        observation,
    )
...
async def test_reconcilable_children_rotate_by_durable_poll_cursor() -> None:
    ...
    pool = CapturePool()
    assert await PostgresDeliveryExecutionRepository(pool).list_reconcilable(limit=5) == []
...
async def test_list_deadline_expired_children_filters_by_deadline_and_terminal_states() -> None:
    ...
    result = await PostgresDeliveryExecutionRepository(pool).list_deadline_expired_children(
        now=now, limit=25
    )
...
async def test_list_deadline_expired_executions_filters_by_deadline_and_terminal_states() -> None:
    ...
    result = await PostgresDeliveryExecutionRepository(pool).list_deadline_expired_executions(
        now=now, limit=10
    )
...
async def test_failure_projection_atomically_preserves_reason_and_cancels_children() -> None:
    ...
    await PostgresDeliveryExecutionRepository(_Pool(connection)).update_execution_state(
        execution.id,
        state=ExecutionState.FAILED.value,
...
async def test_record_reconcile_error_rejects_non_positive_threshold() -> None:
    repository = PostgresDeliveryExecutionRepository(_ResultPool())
...
async def test_evidence_seal_and_join_reject_disappeared_rows() -> None:
    execution = _execution()
    repository = PostgresDeliveryExecutionRepository(_ResultPool(result="UPDATE 0"))
    ...
    ready = await PostgresDeliveryExecutionRepository(
...
async def test_list_parent_stop_pending_orders_fresh_rows_before_failing_ones() -> None:
    ...
    result = await PostgresDeliveryExecutionRepository(pool).list_parent_stop_pending(limit=3)
...
async def test_record_parent_stop_error_increments_and_records_reason() -> None:
    ...
    await PostgresDeliveryExecutionRepository(pool).record_parent_stop_error(
...
async def test_request_cancel_is_atomic_idempotent_and_owner_scoped() -> None:
    ...
    canceled = await PostgresDeliveryExecutionRepository(_Pool(connection)).request_cancel(
        execution.id,
        owner_id=execution.owner_id,
        tenant_id=execution.tenant_id,
    )
    ...
    unchanged = await PostgresDeliveryExecutionRepository(
    ...
    unchanged_failure = await PostgresDeliveryExecutionRepository(
    ...
    recovered_cancel = await PostgresDeliveryExecutionRepository(
    ...
    with pytest.raises(ExecutionConflictError, match="does not exist"):
        await PostgresDeliveryExecutionRepository(
