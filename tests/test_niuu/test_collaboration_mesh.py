...
@pytest.mark.asyncio
async def test_mesh_bridge_filters_session_and_routes_usage_separately() -> None:
    subscriber = _Subscriber()
    handle = AsyncMock()
    usage = AsyncMock()
    bridge = MeshCollaborationBridge(
    ...
    usage.assert_awaited_once_with({"model": "codex"})
    handle.assert_awaited_once()
    assert handle.await_args.args[1]["events"] == [{"kind": "activity", "activityType": "thinking"}]
