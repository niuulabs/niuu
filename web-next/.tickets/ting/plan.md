...
**Visual test:** `e2e/visual/ting.visual.spec.ts` → `ting plan matches web2`
**Status:** FAIL
**Web2 baseline:** `e2e/__screenshots__/web2/ting/plan.png`
**Web2 source:** `web2/niuu_handoff/ting/design/pages.jsx` (PlanView, lines 692-947)
**Web-next source:** `packages/plugin-ting/src/ui/PlanWizard.tsx`, `packages/plugin-ting/src/ui/PlanPrompt.tsx`, `packages/plugin-ting/src/ui/PlanQuestions.tsx`, `packages/plugin-ting/src/ui/PlanRaiding.tsx`, `packages/plugin-ting/src/ui/PlanDraft.tsx`, `packages/plugin-ting/src/ui/PlanApproved.tsx`, `packages/plugin-ting/src/ui/PlanGuidanceRail.tsx`

---

## Summary

The Plan wizard is structurally well-matched: 5-step flow (prompt, questions, runing, draft, approved), StepDots progress indicator, 2-column layout on steps 1/2/4 (content left, guidance rail right), full-width on steps 3/5, and right-side guidance cards. The gaps are mostly cosmetic: hint chips on the prompt step, the "Your Brief" quote card on questions step, workflow template picker styling, sub-tasks table in draft, risk rows styling, and the "Re-plan" button in draft actions.
...
- `packages/plugin-ting/src/ui/PlanPrompt.tsx` — add hint chip buttons
...
- `packages/plugin-ting/src/ui/PlanQuestions.tsx` — add quote card (needs prompt passed as prop)
- `packages/plugin-ting/src/ui/PlanWizard.tsx` — pass prompt to PlanQuestions if not already
...
- `packages/plugin-ting/src/ui/PlanQuestions.tsx` — add workflow picker grid
- `packages/plugin-ting/src/ui/useWorkflows.ts` — reuse existing hook for template list
...
- `packages/plugin-ting/src/ui/PlanDraft.tsx` — match sub-task row layout and size pills
...
- `packages/plugin-ting/src/ui/PlanDraft.tsx` — style risk rows with kind badges

---

### 6. Draft action bar — add "Re-plan" and "Save as draft" buttons

**Web2 spec**: Lines 940-946 show a draft action bar with: "Revise answers" (back), "Re-plan" (restart planning run), spacer, "Save as draft", and "Approve & create saga" (primary). Five buttons total.

**Web-next currently**: `PlanDraft.tsx` likely has Back and Approve buttons. "Re-plan" and "Save as draft" may be missing.

**What to do**: Add "Re-plan" button (restarts the planning run with same inputs) and "Save as draft" button (persists current state without creating the saga). Place them matching web2's layout: back left, re-plan left-center, save-draft right, approve right.

**Files to modify:**

- `packages/plugin-ting/src/ui/PlanDraft.tsx` — add Re-plan and Save as draft buttons
- `packages/plugin-ting/src/ui/usePlanWizard.ts` — add `replan()` and `saveDraft()` actions if missing

---

### 7. Runing animation matches web2

**Web2 spec**: The runing step (lines 838-846) shows a `<RuningAnim/>` component (pulsing animation) and 3 lines of mono text showing what the ravens are doing: decomposer analyzing, investigator probing, mimir-indexer pulling.

**Web-next currently**: `PlanRaiding.tsx` shows a loading/progress state. The specific raven activity lines may be missing.

**What to do**: Ensure the runing step shows descriptive raven activity text (3 lines with rune prefix) below the loading animation.

**Files to modify:**

- `packages/plugin-ting/src/ui/PlanRaiding.tsx` — add raven activity text lines
...
- `StepDots` — plugin-local in `packages/plugin-ting/src/ui/StepDots.tsx`
...
- [ ] Runing step shows 3 raven activity description lines with rune prefixes
- [ ] Visual regression test `ting plan matches web2` passes
