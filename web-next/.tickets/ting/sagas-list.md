...
**Visual test:** `e2e/visual/ting.visual.spec.ts` → `ting sagas list matches web2`
**Status:** FAIL
**Web2 baseline:** `e2e/__screenshots__/web2/ting/sagas.png`
**Web2 source:** `web2/niuu_handoff/ting/design/pages.jsx` (SagasView, lines 266-441)
**Web-next source:** `packages/plugin-ting/src/ui/SagasPage.tsx`

---

## Summary

The Sagas List uses a left panel + right detail split layout in both web2 and web-next. Web-next's left panel is functionally complete (search, status filter tabs, saga rows with glyph/name/meta/pipe). The right detail panel embeds `SagaDetailPage` which shows phases and runs. However, web2's detail panel includes three additional cards on the right side: a workflow card, a stage progress rail, and a confidence drift sparkline chart. These are absent in web-next.
...
**Web-next currently**: `SagaDetailPage.tsx` renders phases and runs in a single column. There is no workflow card. The saga domain model likely does not carry workflow/version info.
...
- `packages/plugin-ting/src/domain/saga.ts` — add `workflow?: string`, `workflowVersion?: string` fields
- `packages/plugin-ting/src/ui/SagaDetailPage.tsx` — add WorkflowCard in a right-column layout
- `packages/plugin-ting/src/ui/SagaDetailPage.test.tsx` — test new card rendering
...
- `packages/plugin-ting/src/ui/StageProgressRail.tsx` — new component
- `packages/plugin-ting/src/ui/StageProgressRail.test.tsx` — tests
- `packages/plugin-ting/src/ui/SagaDetailPage.tsx` — integrate StageProgressRail
...
- A sparkline showing how confidence has moved over time as runs reported back
...
- `packages/plugin-ting/src/ui/ConfidenceDriftCard.tsx` — new component
- `packages/plugin-ting/src/ui/ConfidenceDriftCard.test.tsx` — tests
- `packages/plugin-ting/src/ui/SagaDetailPage.tsx` — integrate card

---

### 4. Convert detail to 2-column layout (content left, cards right)

**Web2 spec**: The saga detail area uses a 2-column layout: left column shows phases/runs, right column shows the three cards (workflow, stage progress, confidence drift). Implemented as `.saga-detail { display: grid; grid-template-columns: 1fr 320px; }`.

**Web-next currently**: `SagaDetailPage` uses a single-column `niuu-p-6 niuu-space-y-6` layout.

**What to do**: Restructure the detail page layout into a 2-column grid. The left column contains header + phases/runs. The right column contains the three cards (workflow, stage progress, confidence drift) stacked vertically.

**Files to modify:**

- `packages/plugin-ting/src/ui/SagaDetailPage.tsx` — wrap content in 2-column grid
...
- `packages/plugin-ting/src/ui/SagasPage.tsx` — add toast on export
...
- `StageProgressRail` — new, plugin-local (only Ting uses it)
...
- [ ] Visual regression test `ting sagas list matches web2` passes
