...
**Visual test:** `e2e/visual/ting.visual.spec.ts` → `ting dispatch matches web2`
**Status:** FAIL
**Web2 baseline:** `e2e/__screenshots__/web2/ting/dispatch.png`
**Web2 source:** `web2/niuu_handoff/ting/design/pages.jsx` (DispatchView, lines 443-690)
**Web-next source:** `packages/plugin-ting/src/ui/DispatchView.tsx`

---

## Summary

The Dispatch page is well-implemented in web-next: split layout (queue left, rules right), segmented filter with 4 tabs, batch dispatch bar, saga group headers, run rows with feasibility gate chips, and a search input. Web-next adds improvements over web2 (feasibility gate chips with tooltips, confidence bar visualization). Three modals from web2 are missing: workflow override, threshold override, and edit rules. The right panel also lacks the "Recent dispatches" data and the "Edit" button for rules.
...
**Web2 spec**: When runs are selected, clicking "Apply workflow..." opens a modal (`modal === 'apply-workflow'`) showing a list of available workflow templates. Each template row shows name, summary/stage count, and version. Clicking a template applies it as an override to the selected runs' sagas.
...
- On selection, applies the workflow override to the sagas of selected runs
...
- `packages/plugin-ting/src/ui/WorkflowOverrideModal.tsx` — new component
- `packages/plugin-ting/src/ui/WorkflowOverrideModal.test.tsx` — tests
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add modal state, wire `onApplyWorkflow`
...
- `packages/plugin-ting/src/ui/ThresholdOverrideModal.tsx` — new component
- `packages/plugin-ting/src/ui/ThresholdOverrideModal.test.tsx` — tests
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add modal state, wire `onOverrideThreshold`

---

### 3. Add "Edit rules" modal with full rules form

**Web2 spec**: The right panel's dispatch rules card has an "Edit" button (line 560) that opens an `EditRulesModal` (lines 673-689) with inputs for: confidence threshold, max concurrent runs, auto-continue toggle, retry count. Cancel and Save buttons.
...
- `packages/plugin-ting/src/ui/EditRulesModal.tsx` — new component
- `packages/plugin-ting/src/ui/EditRulesModal.test.tsx` — tests
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add Edit button to rules panel, modal state

---

### 4. Populate "Recent dispatches" panel with data

**Web2 spec**: The right column's "Recent dispatches" card (lines 569-585) shows a list of recent dispatch events: run identifier, workflow name, and timestamp. Web2 uses 5 hardcoded entries.
...
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add mock data and render in `DispatchRulesPanel`
...
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add pause button in header
- `packages/plugin-ting/src/ports/index.ts` — ensure `IDispatchBus` has a `togglePause()` method

---

### 6. Toast notifications for dispatch actions

**Web2 spec**: Dispatching runs shows `flash("Dispatched N runs")`. Applying workflow shows `flash("Applied 'ship' to N runs")`. Threshold change shows `flash("Threshold -> 0.80")`.
...
- `packages/plugin-ting/src/ui/DispatchView.tsx` — add toast triggers on actions
...
- [ ] Visual regression test `ting dispatch matches web2` passes
