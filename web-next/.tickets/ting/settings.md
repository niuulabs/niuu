...
**Visual test:** `e2e/visual/ting.visual.spec.ts` → `ting settings matches web2`
**Status:** FAIL
**Web2 baseline:** `e2e/__screenshots__/web2/ting/settings.png`
**Web2 source:** `web2/niuu_handoff/ting/design/settings_plugin.jsx` (TING_SECTIONS, lines 71-190)
**Web-next source:** `packages/plugin-ting/src/ui/settings/SettingsPage.tsx`, `packages/plugin-ting/src/ui/settings/PersonasSection.tsx`, `packages/plugin-ting/src/ui/settings/FlockConfigSection.tsx`, `packages/plugin-ting/src/ui/settings/DispatchDefaultsSection.tsx`, `packages/plugin-ting/src/ui/settings/NotificationsSection.tsx`, `packages/plugin-ting/src/ui/settings/AuditLogSection.tsx`

---

## Summary

Web2's Ting settings surface has 8 sections: General, Dispatch rules, Integrations, Persona overrides, Gates & reviewers, Notifications, Advanced, plus 2 Niuu-level sections (Workspace, Appearance). Web-next has 5 sections: Personas, Flock Config, Dispatch Defaults, Notifications, Audit Log. Three Ting sections are missing (General, Integrations, Gates & reviewers) and one is missing entirely (Advanced). The naming also differs — web2 uses "Dispatch rules" vs web-next's "Dispatch Defaults", and web2's "Persona overrides" vs web-next's "Personas". The cross-plugin aggregated nature of settings (where each plugin contributes sections) is designed in web2 but not yet implemented in web-next.
...
**Web2 spec**: `ting.general` section (lines 73-85) shows core service bindings:

- Service URL: `https://ting.niuu.internal`
...
- `packages/plugin-ting/src/ui/settings/GeneralSection.tsx` — new component
- `packages/plugin-ting/src/ui/settings/GeneralSection.test.tsx` — tests
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — add to section list and switch

---

### 2. Add "Integrations" section

**Web2 spec**: `ting.integrations` section (lines 103-127) shows a list of integration cards:
...
- `packages/plugin-ting/src/ui/settings/IntegrationsSection.tsx` — new component
- `packages/plugin-ting/src/ui/settings/IntegrationsSection.test.tsx` — tests
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — add to section list and switch
...
**Web2 spec**: `ting.gates` section (lines 150-162) shows gate reviewer routing:
...
- `packages/plugin-ting/src/ui/settings/GatesReviewersSection.tsx` — new component
- `packages/plugin-ting/src/ui/settings/GatesReviewersSection.test.tsx` — tests
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — add to section list and switch
...
**Web2 spec**: `ting.advanced` section (lines 178-189) shows destructive actions:
...
- `packages/plugin-ting/src/ui/settings/AdvancedSection.tsx` — new component
- `packages/plugin-ting/src/ui/settings/AdvancedSection.test.tsx` — tests
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — add to section list and switch
...
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — rename labels in SECTION_ITEMS
...
- Max concurrent runs: `<input defaultValue="5">`
...
- `packages/plugin-ting/src/ui/settings/DispatchDefaultsSection.tsx` — add missing fields, ensure editable inputs
...
- `packages/plugin-ting/src/ui/settings/PersonasSection.tsx` — add budget/model chips + Edit button
...
- `packages/plugin-ting/src/ui/settings/SettingsPage.tsx` — reorder SECTION_ITEMS
- `packages/plugin-ting/src/ui/settings/SettingsRail.tsx` — ensure nav renders in correct order
...
- [ ] Visual regression test `ting settings matches web2` passes
