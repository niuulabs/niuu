...
const STEP_LABELS: Record<string, string> = { source: 'Source', runtime: 'Runtime', confirm: 'Confirm' };
export const BOOT_STEPS = [
  { id: 'schedule', label: 'schedule pod' }, { id: 'pull', label: 'pull image' },
  { id: 'creds', label: 'check credentials' }, { id: 'clone', label: 'clone workspace' },
  { id: 'mount', label: 'attach PVCs' }, { id: 'mcp', label: 'bring MCP servers up' },
  { id: 'cli', label: 'boot CLI tool' }, { id: 'ready', label: 'ready' },
];
...
export function WizardSelect({ options, value, onChange, placeholder, testId }: {
  options: Array<{ value: string; label: string }>; value: string;
  onChange: (value: string) => void; placeholder?: string; testId?: string;
}) {
  return (
...
export interface LaunchWizardProps { open: boolean; onOpenChange: (open: boolean) => void; initialLaunchSpecRef?: string; }
    ...
    <div className="niuu:flex niuu:items-center niuu:gap-2 niuu:py-4" data-testid="step-indicator">
      {steps.map((step, i) => (
        <div key={step} className="niuu:flex niuu:items-center niuu:gap-2">
          <div className={`niuu:flex niuu:h-6 niuu:w-6 niuu:items-center niuu:justify-center niuu:rounded-full niuu:font-mono niuu:text-xs ${i < idx ? 'niuu:bg-brand niuu:text-bg-primary' : i === idx ? 'niuu:border-2 niuu:border-brand niuu:text-brand' : 'niuu:border niuu:border-border-subtle niuu:text-text-faint'}`} data-testid={`step-${step}`}>
            {i < idx ? '\u2713' : i + 1}
          </div>
          <span className={`niuu:text-xs ${i === idx ? 'niuu:text-text-primary' : 'niuu:text-text-faint'}`}>{STEP_LABELS[step]}</span>
          {i < steps.length - 1 && <div className={`niuu:h-px niuu:w-8 ${i < idx ? 'niuu:bg-brand' : 'niuu:bg-border-subtle'}`} />}
        </div>
      ))}
    </div>
  );
...
export function RuntimePanel({ title, description, children }: { title: string; description?: string; children: ReactNode }) {
  return (
