...
const EMPTY: MemoryEscapableState = { path: [], focus: null, asOf: null };

describe('escapeStep', () => {
  it('clears the traced path first when present', () => {
    expect(escapeStep({ path: ['/a', '/b'], focus: '/b', asOf: '2026-01-01' })).toBe('path');
  });
  it('clears focus next when no path is traced', () => {
    expect(escapeStep({ ...EMPTY, focus: '/b', asOf: '2026-01-01' })).toBe('focus');
  });
