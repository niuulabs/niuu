import type {
  DeveloperCandidate,
  DeveloperChildExecution,
  DeveloperDeliveryWait,
  DeveloperExecution,
  DeveloperReviewReceipt,
  DeveloperVerificationReceipt,
...
function candidateFor(child: DeveloperChildExecution): DeveloperCandidate | null {
  return child.candidate ?? null;
}

function verificationRows(receipts: DeveloperVerificationReceipt[]): string[] {
...
function reviewRows(receipts: DeveloperReviewReceipt[]): string[] {
  return receipts.map(
    (receipt) =>
...
function workstreamSummary(child: DeveloperChildExecution): string {
...
function currentAndHistoricalChildren(execution: DeveloperExecution): {
  current: DeveloperChildExecution[];
  historical: DeveloperChildExecution[];
} {
  const eligible = execution.children.filter(
    (child) => child.generation === undefined || child.generation === execution.currentGeneration,
  );
  const latest = new Map<string, DeveloperChildExecution>();
...
function requirementLines(candidate: DeveloperCandidate): string[] {
...
function workstreamDetails(child: DeveloperChildExecution): string[] {
  const candidate = candidateFor(child);
...
function blockerLines(
  ...
  currentChildren: DeveloperChildExecution[],
...
function deliveryWaitLines(
  waits: readonly DeveloperDeliveryWait[],
  identity: CurrentDeliveryIdentity | null,
...
export function buildDeveloperExecutionResultsMarkdown(
  execution: DeveloperExecution,
  evidence: Record<string, unknown>,
  deliveryWaits: readonly DeveloperDeliveryWait[] = [],
): string {
