...
interface PlanDraftProps {
  ...
  onRemoveRun?(phaseIndex: number, runIndex: number): void;
}
...
interface PhaseEditorProps {
  ...
  onRemoveRun?(runIndex: number): void;
}

function PhaseEditor({ phase, phaseIndex, onSave, onRemoveRun }: PhaseEditorProps) {
        ...
        <ul className="niuu:flex niuu:flex-col niuu:gap-2 niuu:list-none niuu:p-0 niuu:m-0">
          {phase.runs.map((run, ri) => (
            ...
            >
              ...
              >
                {run.persona ? run.persona.charAt(0).toUpperCase() : 'ᚱ'}
              </span>
              ...
              <div className="niuu:flex niuu:flex-col niuu:gap-0.5 niuu:min-w-0">
                ...
                <span className="niuu:text-xs niuu:text-text-muted niuu:font-mono">
                  ...
                  {!run.phase &&
                    !run.persona &&
                    `~${run.estimateHours}h · ${run.confidence}% confidence`}
                </span>
              </div>
...
export function PlanDraft({
  ...
  const totalRuns = phases.reduce((sum, p) => sum + p.runs.length, 0);
  const avgConfidence =
    totalRuns > 0
      ? Math.round(
          phases.flatMap((p) => p.runs).reduce((sum, r) => sum + r.confidence, 0) / totalRuns,
        )
      : 0;
      ...
      <div className="niuu:flex niuu:flex-col niuu:gap-3">
        {phases.map((phase, idx) => (
          <PhaseEditor
            ...
            onRemoveRun={onRemoveRun ? (ri) => onRemoveRun(idx, ri) : undefined}
          />
        ))}
      </div>
