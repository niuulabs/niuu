...
function wrap(dispatch: IDispatchBus) {
  ...
  return function Wrapper({ children }: { children: React.ReactNode }) {
    return createElement(
      QueryClientProvider,
      { client },
      createElement(ServicesProvider, { services: { 'ting.dispatch': dispatch } }, children),
    );
  };
}
...
describe('useDispatchQueue', () => {
  it('returns entries from the dispatcher queue', async () => {
    const dispatch: IDispatchBus = {
      ...
      getQueue: async () => [
        makeQueueItem({ issueId: 'issue-1', title: 'Run 1' }),
        makeQueueItem({ issueId: 'issue-2', identifier: 'NIU-124', title: 'Run 2' }),
      ],
    };
    ...
    expect(result.current.data?.[0]?.run.id).toBe('issue-1');
    expect(result.current.data?.[0]?.run.name).toBe('Run 1');
