...
type Edge = [RavenNode, RavenNode, RunCluster];
...
export interface RunMeshCanvasProps {
  sagas: Saga[];
  phases: Phase[][];
...
export function RunMeshCanvas({
  sagas,
  phases,
  onClickSaga,
  'aria-label': ariaLabel = 'Live run mesh visualization',
}: RunMeshCanvasProps) {
  ...
  // Build run clusters from sagas + phases — memoized to avoid per-render churn
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      clusterNodes.forEach((n) => {
        ...
        ctx.fillText(n.cluster.runName.slice(0, 12), n.x, n.y + 4);
