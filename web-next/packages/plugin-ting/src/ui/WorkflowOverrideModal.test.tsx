  ...
  return function Wrapper({ children }: { children: React.ReactNode }) {
    return (
      <QueryClientProvider client={client}>
        <ServicesProvider services={{ 'ting.workflows': workflowService }}>
          {children}
        </ServicesProvider>
      </QueryClientProvider>
    );
...
describe('WorkflowOverrideModal', () => {
  ...
  it('shows description with selected count', () => {
    ...
    expect(screen.getByText(/3 selected runs/i)).toBeInTheDocument();
  });

  it('shows singular "run" for count=1', () => {
    render(
      <WorkflowOverrideModal
        open={true}
        onOpenChange={vi.fn()}
    ...
    expect(screen.getByText(/1 selected run[^s]/i)).toBeInTheDocument();
  });
