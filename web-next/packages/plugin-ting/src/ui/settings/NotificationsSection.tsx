...
export function NotificationsSection() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const patch: Partial<Omit<NotificationSettings, 'updatedAt'>> = {
      ...
      onRunPendingApproval: data.get('onRunPendingApproval') === 'true',
      onRunMerged: data.get('onRunMerged') === 'true',
      onRunFailed: data.get('onRunFailed') === 'true',
      ...
      >
        ...
        {effectiveChannel === 'webhook' && (
          <Field id="notif-webhook-url" label="Webhook URL" required>
            <Input
              ...
              placeholder="https://example.com/hooks/ting"
            />
          </Field>
        )}
        ...
        <div className="niuu-border niuu-border-border niuu-rounded-md niuu-px-3 niuu-py-2">
          ...
          <ToggleRow
            ...
            name="onRunPendingApproval"
            label="Run awaiting approval"
            defaultChecked={settings?.onRunPendingApproval}
          />
          <ToggleRow
            id="notif-run-merged"
            name="onRunMerged"
            label="Run merged"
            defaultChecked={settings?.onRunMerged}
          />
          <ToggleRow
            id="notif-run-failed"
            name="onRunFailed"
            label="Run failed"
            defaultChecked={settings?.onRunFailed}
          />
          <ToggleRow
