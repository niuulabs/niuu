...
describe('NodeInspector — stage node', () => {
  ...
  it('shows the run ID', () => {
    render(<NodeInspector node={stageNode} {...defaultHandlers()} />);
    expect(screen.getByTestId('inspector-run-id')).toHaveTextContent('run-abc');
  });

  it('shows "unassigned" when runId is null', () => {
    const node = { ...stageNode, runId: null };
    render(<NodeInspector node={node} {...defaultHandlers()} />);
    expect(screen.getByTestId('inspector-run-id')).toHaveTextContent('unassigned');
  });
...
describe('NodeInspector — gate node', () => {
  ...
  it('does not show persona section for gate node', () => {
    render(<NodeInspector node={gateNode} {...defaultHandlers()} />);
    expect(screen.queryByTestId('inspector-run-id')).toBeNull();
  });
});
