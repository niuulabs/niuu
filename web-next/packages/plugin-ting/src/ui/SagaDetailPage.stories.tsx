...
function StoryWrapper({
  ...
  const router = createRouter({
    ...
    history: createMemoryHistory({
      initialEntries: [`/ting/sagas/${STORY_SAGA_ID}`],
    }),
  });
...
const meta: Meta<typeof SagaDetailPage> = {
  title: 'Plugins / Ting / SagaDetailPage',
  component: SagaDetailPage,
  parameters: { layout: 'fullscreen' },
  decorators: [
    (Story) => (
      <StoryWrapper services={{ ting: createMockTingService() }}>
        <Story />
      </StoryWrapper>
    ),
  ],
...
/** Default — Auth Rewrite saga with 3 phases and 2 runs from seed data. */
