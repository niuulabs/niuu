...
import type { IDeveloperExecutionService, IWorkflowService } from '../ports';
import type { DeveloperExecution, DeveloperExecutionLaunch } from '../domain/developerExecution';
import { executionIsTerminal } from '../domain/developerExecution';
import { buildDeveloperExecutionResultsMarkdown } from '../application/developerExecutionResults';
...
export function DeveloperExecutionsPage() {
  ...
  const runs = useInfiniteQuery({
    queryKey: ['ting', 'developer-executions', 'pages'],
  ...
  const selected = useQuery({
    queryKey: ['ting', 'developer-execution', selectedId],
  ...
  const update = (run: DeveloperExecution) => {
    setSelectedId(run.executionId);
    queryClient.setQueryData(['ting', 'developer-execution', run.executionId], run);
    void queryClient.invalidateQueries({ queryKey: ['ting', 'developer-executions'] });
  ...
  const launch = useMutation({
    mutationFn: (request: DeveloperExecutionLaunch) => {
              ...
              deliveryWaits.data && (
                <div ref={evidenceRegion} tabIndex={-1} className="niuu:space-y-3">
                  <WorkflowResults
                    ...
                    markdown={buildDeveloperExecutionResultsMarkdown(
                      selected.data,
                      evidence.data,
                      deliveryWaits.data,
                    )}
