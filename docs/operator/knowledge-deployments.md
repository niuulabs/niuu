...
schema migrations. The application remains non-superuser; CloudNativePG generates its credential.
...
A one-shot chart Job also installs gbrain's required PostgreSQL event trigger
as the database administrator. Its credential is generated by CloudNativePG
and is referenced only by that Job. The gbrain service and dreams keep using
the non-superuser application credential. The idempotent Job handles existing
databases as well as new installs; it never replaces an existing trigger. The application owns the trigger function
so subsequent gbrain migrations can harden it; the event trigger remains
administrator-owned.
